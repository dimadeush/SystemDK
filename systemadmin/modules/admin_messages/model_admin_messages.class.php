...
class admin_messages extends model_base
{
    ...
    public function index($num_page)
    {
        ...
        if ($result) {
            ...
            $sql = "SELECT message_id,message_author,message_title,message_term_expire,message_status,message_value_view FROM " . PREFIX . "_messages_"
                   . $this->registry->sitelang . " order by message_date DESC";
        ...
        if ($result) {
            ...
            if ($row_exist > 0) {
                while (!$result->EOF) {
                    ...
                    if (!isset($message_termexpire) || $message_termexpire == "" || $message_termexpire == 0) {
    ...
    public function message_add_inbase($array)
    {
        ...
        if ((!isset($array['message_title']) || !isset($array['message_content']) || !isset($array['message_notes']) || !isset($array['message_status'])
             || !isset($array['message_termexpire'])
             || !isset($array['message_termexpireaction'])
             || !isset($array['message_valueview']))
            || ($data_array['message_title'] == "" || $data_array['message_content'] == "")
        ) {
        ...
        if ($check_db_need_lobs == 'yes') {
            ...
            $sql = "INSERT INTO " . PREFIX . "_messages_" . $this->registry->sitelang
                   . " (message_id,message_author,message_title,message_date,message_content,message_notes,message_value_view,message_status,message_term_expire,message_term_expire_action)  VALUES ('"
                   . $message_id . "'," . $message_author . "," . $data_array['message_title'] . ",'" . $now . "',empty_clob(),empty_clob(),'"
                   . $data_array['message_valueview'] . "','" . $data_array['message_status'] . "'," . $data_array['message_termexpire'] . ","
                   . $data_array['message_termexpireaction'] . ")";
            $result = $this->db->Execute($sql);
            if ($result === false) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
            ...
            $result2 = $this->db->UpdateClob(
                PREFIX . '_messages_' . $this->registry->sitelang, 'message_content', $data_array['message_content'], 'message_id = ' . $message_id
            );
            if ($result2 === false) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
            ...
            if ($data_array['message_notes'] != 'NULL') {
                ...
                $result3 = $this->db->UpdateClob(
                    PREFIX . '_messages_' . $this->registry->sitelang, 'message_notes', $data_array['message_notes'], 'message_id = ' . $message_id
                );
                if ($result3 === false) {
                    $error_message = $this->db->ErrorMsg();
                    $error_code = $this->db->ErrorNo();
            ...
            $sql = "INSERT INTO " . PREFIX . "_messages_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
                   . "message_author,message_title,message_date,message_content,message_notes,message_value_view,message_status,message_term_expire,message_term_expire_action)  VALUES ("
                   . $sequence_array['sequence_value_string'] . "" . $message_author . "," . $data_array['message_title'] . ",'" . $now . "','"
                   . $data_array['message_content'] . "'," . $data_array['message_notes'] . ",'" . $data_array['message_valueview'] . "','"
                   . $data_array['message_status']
                   . "'," . $data_array['message_termexpire'] . "," . $data_array['message_termexpireaction'] . ")";
            $result = $this->db->Execute($sql);
            if ($result === false) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
    ...
    public function message_status($array)
    {
        ...
        if ($message_id == 0 && isset($array['post_message_id']) && is_array($array['post_message_id']) && count($array['post_message_id']) > 0) {
            $message_id_is_arr = 1;
        ...
        if (!isset($message_id) || $action == "" || $message_id == "'0'") {
    ...
    public function message_edit($message_id)
    {
        ...
        $sql =
            "SELECT message_id,message_author,message_title,message_date,message_content,message_notes,message_value_view,message_status,message_term_expire,message_term_expire_action FROM "
            . PREFIX . "_messages_" . $this->registry->sitelang . " WHERE message_id = '" . $message_id . "'";
        $result = $this->db->Execute($sql);
        if ($result) {
            if (isset($result->fields['0'])) {
                $row_exist = intval($result->fields['0']);
        ...
        if (isset($result->fields['8']) && intval($result->fields['8']) !== "") {
    ...
    public function message_edit_inbase($array)
    {
        ...
        if ((!isset($array['message_id']) || !isset($array['message_author']) || !isset($array['message_title']) || !isset($array['message_content'])
             || !isset($array['message_notes'])
             || !isset($array['message_status'])
             || !isset($array['message_termexpire'])
             || !isset($array['message_termexpireaction'])
             || !isset($array['message_valueview']))
            || ($data_array['message_id'] == 0 || $data_array['message_author'] == "" || $data_array['message_title'] == "" || $data_array['message_content'] == "")
        ) {
        ...
        if ($check_db_need_lobs == 'yes') {
            $this->db->StartTrans();
            $sql = "UPDATE " . PREFIX . "_messages_" . $this->registry->sitelang . " SET message_author = " . $data_array['message_author'] . ",message_title = "
                   . $data_array['message_title'] . ",message_content = empty_clob(),message_notes = empty_clob(),message_status = '" . $data_array['message_status']
                   . "',message_term_expire = " . $data_array['message_termexpire'] . ",message_term_expire_action = " . $data_array['message_termexpireaction']
                   . ",message_value_view = '" . $data_array['message_valueview'] . "' WHERE message_id = '" . $data_array['message_id'] . "'";
            $result = $this->db->Execute($sql);
            $num_result = $this->db->Affected_Rows();
            if ($result === false) {
                $error_message = $this->db->ErrorMsg();
            ...
            $result2 = $this->db->UpdateClob(
                PREFIX . '_messages_' . $this->registry->sitelang, 'message_content', $data_array['message_content'], 'message_id = ' . $data_array['message_id']
            );
            if ($result2 === false) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
            ...
            if ($data_array['message_notes'] != 'NULL') {
                $data_array['message_notes'] = substr(substr($data_array['message_notes'], 0, -1), 1);
                $result3 = $this->db->UpdateClob(
                    PREFIX . '_messages_' . $this->registry->sitelang, 'message_notes', $data_array['message_notes'], 'message_id = ' . $data_array['message_id']
                );
                if ($result3 === false) {
                    $error_message = $this->db->ErrorMsg();
                    $error_code = $this->db->ErrorNo();
            ...
            $sql = "UPDATE " . PREFIX . "_messages_" . $this->registry->sitelang . " SET message_author = " . $data_array['message_author'] . ",message_title = "
                   . $data_array['message_title'] . ",message_content = '" . $data_array['message_content'] . "',message_notes = " . $data_array['message_notes']
                   . ",message_status = '" . $data_array['message_status'] . "',message_term_expire = " . $data_array['message_termexpire']
                   . ",message_term_expire_action = " . $data_array['message_termexpireaction'] . ",message_value_view = '" . $data_array['message_valueview']
                   . "' WHERE message_id = '" . $data_array['message_id'] . "'";
            $result = $this->db->Execute($sql);
            $num_result = $this->db->Affected_Rows();
            if ($result === false) {
                $error_message = $this->db->ErrorMsg();
