...
class controller_admin_blocks extends controller_base
{
    ...
    public function blockorder()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'empty_data' || $error === 'show_sql_error' || $error === 'order_ok') {
            if ($error === 'empty_data') {
    ...
    public function add_block()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'add_not_all_data' || $error === 'add_sql_error' || $error === 'add_no_file' || $error == 'unknown_block_type') {
            ...
            $cache_category = false;
            $template = false;
            if ($error === 'add_not_all_data' || $error === 'add_no_file') {
    ...
    public function add_block_inbase()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'add_not_all_data' || $error === 'add_found' || $error === 'add_sql_error' || $error === 'error_url' || $error === 'unknown_block_type') {
            if ($error === 'unknown_block_type') {
                header("Location: index.php?path=admin_blocks&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
            ...
            $cache_category = false;
            $template = false;
            if ($error === 'add_not_all_data' || $error === 'add_found' || $error === 'error_url') {
                $template = 'blockaddend.html';
            }
            $this->blocks_view($error, $cache_category, $template, $title);
        }
    ...
    public function block_status()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'unknown_action' || $error === 'status_sql_error' || $error === 'status_ok') {
            if ($error === 'not_all_data' || $error === 'unknown_action') {
                header("Location: index.php?path=admin_blocks&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
            if (!empty($error_array)) {
    ...
    public function block_delete()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'status_not_found' || $error === 'status_sql_error') {
    ...
    public function block_edit()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'edit_sql_error' || $error === 'unknown_block_type' || $error === 'edit_not_found') {
            if ($error === 'not_all_data' || $error === 'unknown_block_type') {
                header("Location: index.php?path=admin_blocks&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
            if (!empty($error_array)) {
    ...
    public function block_edit_save()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'edit_not_all_data' || $error === 'edit_found' || $error === 'edit_block_rss_url' || $error === 'unknown_block_type' || $error === 'edit_sql_error'
            || $error === 'edit_ok'
        ) {
            if ($error === 'unknown_block_type') {
                header("Location: index.php?path=admin_blocks&func=index&lang=" . $this->registry->sitelang);
            ...
            $cache_category = false;
            if ($error === 'edit_not_all_data' || $error === 'edit_found' || $error === 'edit_block_rss_url') {
                $template = 'blockaddend.html';
            }
            $this->blocks_view($error, $cache_category, $template, $title);
        }
    ...
    public function block_rsssite()
    {
        ...
        if (!$this->isCached(
            ...
            $error_array = $this->model_admin_blocks->get_property_value('error_array');
            $result = $this->model_admin_blocks->get_property_value('result');
            if ($error == 'rss_sql_error' || $error === 'unknown_page') {
    ...
    public function block_addrsssite_inbase()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'add_rss_not_all_data' || $error === 'add_rss_sql_error') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    public function block_editrsssite()
    {
        ...
        if (!$this->isCached("systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $rss_id . "|" . $this->registry->language)) {
            ...
            $error_array = $this->model_admin_blocks->get_property_value('error_array');
            $result = $this->model_admin_blocks->get_property_value('result');
            if ($error === 'empty_rss_id' || $error === 'edit_rss_sql_error' || $error === 'edit_rss_not_found') {
    ...
    public function block_editrsssite_inbase()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'edit_rss_not_all_data' || $error === 'edit_rss_sql_error' || $error === 'edit_rss_ok') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    public function block_deletersssite()
    {
        ...
        $error_array = $this->model_admin_blocks->get_property_value('error_array');
        $result = $this->model_admin_blocks->get_property_value('result');
        if ($error === 'empty_rss_id' || $error === 'del_rss_sql_error' || $error === 'del_rss_ok') {
            if ($error == 'empty_rss_id') {
                header("Location: index.php?path=admin_blocks&func=block_rsssite&lang=" . $this->registry->sitelang);
                exit();
            }
