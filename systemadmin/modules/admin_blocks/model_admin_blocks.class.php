...
class admin_blocks extends model_base
{
    ...
    public function index()
    {
        ...
        $sql =
            "SELECT a.block_id, a.block_name, a.block_custom_name, a.block_version, a.block_status, a.block_value_view, a.block_content, a.block_url, a.block_arrangements, a.block_priority, (SELECT max(b.block_id) FROM "
            . PREFIX . "_blocks_" . $this->registry->sitelang
            . " b WHERE b.block_priority = a.block_priority-1 AND b.block_arrangements = a.block_arrangements) as block_id2,(SELECT max(c.block_id) FROM " . PREFIX
            . "_blocks_" . $this->registry->sitelang
            . " c WHERE c.block_priority = a.block_priority+1 AND c.block_arrangements = a.block_arrangements) as block_id3,a.block_term_expire FROM " . PREFIX
            . "_blocks_" . $this->registry->sitelang . " a ORDER BY a.block_arrangements, a.block_priority";
        ...
        if ($result) {
            ...
            if ($row_exist > 0) {
                while (!$result->EOF) {
                    ...
                    if (isset($block_id2) && $block_id2 != 0) {
                        $param1 = $block_id2;
                    } else {
                        $param1 = "";
                    }
                    $block_id3 = intval($result->fields['11']);
                    if (isset($block_id3) && $block_id3 != 0) {
                        $param2 = $block_id3;
                    } else {
                        $param2 = "";
                    }
                    $block_termexpire = intval($result->fields['12']);
                    if (!isset($block_termexpire) || $block_termexpire == 0) {
    ...
    public function blockorder($array)
    {
        ...
        if ((!isset($array['block_id']) || !isset($array['block_newpriority']) || !isset($array['block_priority']) || !isset($array['block_nextid']))
            || ($block_id == 0
                || $block_newpriority == 0
                || $block_priority == 0
                || $block_nextid == 0)
        ...
        $result = $this->db->Execute(
            "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_priority='" . $block_priority . "' WHERE block_id='" . $block_nextid . "'"
        );
        ...
        $result2 = $this->db->Execute(
            "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_priority='" . $block_newpriority . "' WHERE block_id='" . $block_id . "'"
        );
        ...
        if ($result === false || $result2 === false) {
            ...
            return;
        } elseif ($result !== false && $result2 !== false && $num_result1 == 0 && $num_result2 == 0) {
            $this->error = 'order_ok';
    ...
    public function add_block_inbase($array)
    {
        ...
        if ((!isset($array['block_type']) || !isset($array['block_custom_name']) || !isset($array['block_arrangements']) || !isset($array['block_status'])
             || !isset($array['block_termexpire'])
             || !isset($array['block_termexpireaction'])
             || !isset($array['block_valueview'])
             || !isset($array['block_content'])
             || !isset($array['block_url'])
             || !isset($array['block_refresh'])
             || !isset($array['rss_url']))
            || ($data_array['block_type'] == "" || $data_array['block_custom_name'] == "" || $data_array['block_arrangements'] == ""
        ...
        $sql = "SELECT (SELECT MAX(a.block_priority) FROM " . PREFIX . "_blocks_" . $this->registry->sitelang
               . " a WHERE a.block_arrangements='" . $data_array['block_arrangements'] . "') as block_priority,(SELECT MAX(b.block_id) FROM " . PREFIX . "_blocks_"
               . $this->registry->sitelang . " b WHERE b.block_custom_name='" . $data_array['block_custom_name'] . "') as check_block_custom_name "
               . $this->registry->main_class->check_db_need_from_clause();
        $result0 = $this->db->Execute($sql);
        if ($result0) {
            if (isset($result0->fields['0']) && intval($result0->fields['0']) > 0) {
        ...
        if ($data_array['block_type'] == "file") {
            if (empty($data_array['block_file'])) {
                ...
                $sql = "INSERT INTO " . PREFIX . "_blocks_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
                       . "block_name,block_custom_name,block_status,block_value_view,block_arrangements,block_priority,block_term_expire,block_term_expire_action)  VALUES ("
                       . $sequence_array['sequence_value_string'] . "'" . $data_array['block_file'] . "','" . $data_array['block_custom_name'] . "','"
                       . $data_array['block_status'] . "','" . $data_array['block_valueview'] . "','" . $data_array['block_arrangements'] . "','" . $block_priority . "',"
            ...
            if (empty($data_array['block_content'])) {
                ...
                if ($check_db_need_lobs == 'yes') {
                    ...
                    $sql = "INSERT INTO " . PREFIX . "_blocks_" . $this->registry->sitelang
                           . " (block_id,block_content,block_custom_name,block_status,block_value_view,block_arrangements,block_priority,block_term_expire, block_term_expire_action) VALUES ($block_add_id,empty_clob(),'"
                           . $data_array['block_custom_name'] . "','" . $data_array['block_status'] . "','" . $data_array['block_valueview'] . "','"
                           . $data_array['block_arrangements'] . "','" . $block_priority . "'," . $data_array['block_termexpire'] . ","
                           . $data_array['block_termexpireaction'] . ")";
                    ...
                    $result = $this->db->UpdateClob(
                        PREFIX . '_blocks_' . $this->registry->sitelang, 'block_content', $data_array['block_content'], 'block_id=' . $block_add_id
                    );
                    $this->db->CompleteTrans();
                } else {
                    $sql = "INSERT INTO " . PREFIX . "_blocks_" . $this->registry->sitelang
                           . " (block_content,block_custom_name,block_status,block_value_view,block_arrangements,block_priority,block_term_expire, block_term_expire_action)  VALUES ('"
                           . $data_array['block_content'] . "','" . $data_array['block_custom_name'] . "','" . $data_array['block_status'] . "','"
                           . $data_array['block_valueview'] . "','" . $data_array['block_arrangements'] . "','" . $block_priority . "'," . $data_array['block_termexpire']
                           . "," . $data_array['block_termexpireaction'] . ")";
                    $result = $this->db->Execute($sql);
                    $result2 = true;
                }
            ...
            if (empty($data_array['block_url'])) {
                ...
                if ($data_array['block_url'] != "") {
                    ...
                    if (!empty($get_result['rss_link']) && $get_result['rss_link'] === 'no') {
                        $this->error = 'error_url';

                        return;
                    } elseif (!empty($get_result['rss_link']) && $get_result['rss_link'] === 'no2') {
                        $content = "";
                    } elseif ($get_result) {
                        $content = $get_result;
                    }
                ...
                if ($check_db_need_lobs == 'yes') {
                    ...
                    $block_add_id = $this->db->GenID(PREFIX . "_blocks_id_" . $this->registry->sitelang);
                    $sql = "INSERT INTO " . PREFIX . "_blocks_" . $this->registry->sitelang
                           . " (block_id,block_url,block_custom_name,block_status,block_value_view,block_content,block_arrangements,block_priority,block_refresh,block_last_refresh,block_term_expire, block_term_expire_action)  VALUES ("
                           . $block_add_id . "," . $data_array['block_url'] . ",'" . $data_array['block_custom_name'] . "','" . $data_array['block_status'] . "','"
                           . $data_array['block_valueview'] . "',empty_clob(),'" . $data_array['block_arrangements'] . "','" . $block_priority . "','"
                    ...
                    $sql = "INSERT INTO " . PREFIX . "_blocks_" . $this->registry->sitelang
                           . " (block_url,block_custom_name,block_status,block_value_view,block_content,block_arrangements,block_priority,block_refresh,block_last_refresh,block_term_expire, block_term_expire_action)  VALUES ("
                           . $data_array['block_url'] . ",'" . $data_array['block_custom_name'] . "','" . $data_array['block_status'] . "','"
                           . $data_array['block_valueview'] . "','" . $content . "','" . $data_array['block_arrangements'] . "','" . $block_priority . "','"
                           . $data_array['block_refresh'] . "','" . $time . "'," . $data_array['block_termexpire'] . "," . $data_array['block_termexpireaction'] . ")";
                    $result = $this->db->Execute($sql);
                    $result2 = true;
                }
    ...
    public function block_status($array)
    {
        ...
        if ((!isset($array['block_id']) || !isset($array['action'])) || ($block_id == 0 || $action == "")) {
    ...
    public function block_delete($block_id)
    {
        ...
        if ($result) {
            ...
            if ($row_exist > 0) {
                ...
                $this->db->StartTrans();
                $this->db->Execute(
                    "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_priority = block_priority-1 WHERE block_arrangements = '"
                    . $block_arrangements . "' and block_priority > '" . $block_priority . "'"
                );
    ...
    public function block_edit($array)
    {
        ...
        if ((!isset($array['block_id']) || !isset($array['block_type'])) || ($block_id == 0 || $block_type == "")) {
            $this->error = 'not_all_data';
            ...
            for ($i = 0; $i < sizeof($blocks_list); $i++) {
                ...
                if ($blocks_list[$i] != "") {
                    ...
                    if ($result) {
                        ...
                        $blocks_list[$i] = $this->registry->main_class->extracting_data($blocks_list[$i]);
                        if (($row_exist == 0 && $blocks_list[$i] != "") || (intval($result->fields['0']) == $block_id)) {
        ...
        $sql =
            "SELECT block_id,block_name,block_custom_name,block_version,block_status,block_value_view,block_content,block_url,block_arrangements,block_refresh,block_last_refresh,block_term_expire,block_term_expire_action FROM "
            . PREFIX . "_blocks_" . $this->registry->sitelang . " WHERE block_id = '" . $block_id . "'";
        $result = $this->db->Execute($sql);
        if ($result) {
            if (isset($result->fields['0'])) {
                $row_exist = intval($result->fields['0']);
            ...
            if ($row_exist > 0) {
                if (isset($result->fields['11']) && intval($result->fields['11']) != 0 && intval($result->fields['11']) !== "") {
    ...
    public function block_edit_save($array)
    {
        ...
        if ((!isset($array['block_id']) || !isset($array['block_type']) || !isset($array['block_custom_name']) || !isset($array['block_file'])
             || !isset($array['block_arrangements'])
             || !isset($array['block_status'])
             || !isset($array['block_termexpire'])
             || !isset($array['block_termexpireaction'])
             || !isset($array['block_valueview'])
             || !isset($array['block_content'])
             ...
             || !isset($array['block_refresh'])
             || !isset($array['rss_url']))
            || ($data_array['block_type'] == "" || $data_array['block_id'] == 0 || $data_array['block_custom_name'] == "" || $data_array['block_arrangements'] == ""
                || $data_array['block_termexpireaction'] == "")
        ) {
        ...
        $data_array['block_arrangements'] = $this->registry->main_class->format_striptags($data_array['block_arrangements']);
        $data_array['block_arrangements'] = $this->registry->main_class->processing_data($data_array['block_arrangements']);
        $sql = "select a.block_arrangements,(select max(b.block_priority) from " . PREFIX . "_blocks_" . $this->registry->sitelang
               . " b where b.block_arrangements = " . $data_array['block_arrangements'] . ") as block_new_priority,(SELECT max(c.block_id) FROM " . PREFIX . "_blocks_"
               . $this->registry->sitelang . " c WHERE c.block_custom_name = " . $data_array['block_custom_name'] . " and c.block_id <> '" . $data_array['block_id']
               . "') as check_custom_name,a.block_priority from " . PREFIX . "_blocks_" . $this->registry->sitelang . " a where a.block_id = '" . $data_array['block_id']
               . "'";
        $result0 = $this->db->Execute($sql);
        if ($result0 && isset($result0->fields['0']) && isset($result0->fields['3']) && trim($result0->fields['0']) !== '' && intval($result0->fields['3']) != 0) {
            ...
            if (isset($result0->fields['1']) && intval($result0->fields['1']) > 0) {
        ...
        if ($data_array['block_type'] == "file") {
            ...
            $this->db->StartTrans();
            if ($needupdate == "yes") {
                $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_name = " . $data_array['block_file'] . ",block_custom_name = "
                       . $data_array['block_custom_name'] . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                       . "',block_arrangements = '" . $data_array['block_arrangements'] . "',block_priority = '" . $block_new_priority . "',block_term_expire = "
                       . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction'] . " WHERE block_id = '"
                       . $data_array['block_id'] . "'";
                $result2 = $this->db->Execute(
                    "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_priority = block_priority-1 WHERE block_arrangements = '"
                    . $block_old_arrangements . "' and block_priority > '" . $block_old_priority . "'"
                );
            } else {
                $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_name = " . $data_array['block_file'] . ",block_custom_name = "
                       . $data_array['block_custom_name'] . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                       . "',block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction']
                       . " WHERE block_id = '" . $data_array['block_id'] . "'";
                $result2 = true;
            }
            if ($result2 === false) {
                $error_message = $this->db->ErrorMsg();
            ...
            if ($needupdate == "yes") {
                if ($check_db_need_lobs == 'yes') {
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                           . "',block_content = empty_clob(),block_arrangements = '" . $data_array['block_arrangements'] . "',block_priority = '" . $block_new_priority
                           . "', block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction']
                           . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                    ...
                    $result3 = $this->db->UpdateClob(
                        PREFIX . '_blocks_' . $this->registry->sitelang, 'block_content', $data_array['block_content'], 'block_id = ' . $data_array['block_id']
                    );
                    if ($result3 === false) {
                        $error_message = $this->db->ErrorMsg();
                        $error_code = $this->db->ErrorNo();
                    ...
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview'] . "',block_content = '"
                           . $data_array['block_content'] . "',block_arrangements = '" . $data_array['block_arrangements'] . "',block_priority = '" . $block_new_priority
                           . "', block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction']
                           . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                ...
                if ($check_db_need_lobs == 'yes') {
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                           . "',block_content = empty_clob(),block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = "
                           . $data_array['block_termexpireaction'] . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                    if ($result === false) {
                        $error_message = $this->db->ErrorMsg();
                    ...
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview'] . "',block_content = '"
                           . $data_array['block_content'] . "',block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = "
                           . $data_array['block_termexpireaction'] . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                    if ($result === false) {
            ...
            if ($data_array['rss_url'] != "" && $data_array['rss_url'] != "0") {
                $data_array['block_url'] = $data_array['rss_url'];
            }
            $data_array['block_url'] = $this->registry->main_class->format_striptags($data_array['block_url']);
            if (empty($data_array['block_url'])) {
            ...
            if ($needupdate == "yes") {
                if ($check_db_need_lobs == 'yes') {
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                           . "',block_content = empty_clob(),block_url = " . $data_array['block_url'] . ",block_arrangements = '" . $data_array['block_arrangements']
                           . "',block_priority = '" . $block_new_priority . "',block_refresh = '" . $data_array['block_refresh'] . "',block_last_refresh = '" . $time
                           . "',block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction']
                           . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                    ...
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview'] . "',block_content = '"
                           . $content
                           . "',block_url = " . $data_array['block_url'] . ",block_arrangements = '" . $data_array['block_arrangements'] . "',block_priority = '"
                           . $block_new_priority . "',block_refresh = '" . $data_array['block_refresh'] . "',block_last_refresh = '" . $time . "',block_term_expire = "
                           . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction'] . " WHERE block_id = '"
                           . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                ...
                if ($check_db_need_lobs == 'yes') {
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview']
                           . "',block_content = empty_clob(),block_url = " . $data_array['block_url'] . ",block_refresh = '" . $data_array['block_refresh']
                           . "',block_last_refresh = '" . $time . "',block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = "
                           . $data_array['block_termexpireaction'] . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
                    if ($result === false) {
                    ...
                    $sql = "UPDATE " . PREFIX . "_blocks_" . $this->registry->sitelang . " SET block_custom_name = " . $data_array['block_custom_name']
                           . ",block_status = '" . $data_array['block_status'] . "',block_value_view = '" . $data_array['block_valueview'] . "',block_content = '"
                           . $content
                           . "',block_url = " . $data_array['block_url'] . ",block_refresh = '" . $data_array['block_refresh'] . "',block_last_refresh = '" . $time
                           . "',block_term_expire = " . $data_array['block_termexpire'] . ",block_term_expire_action = " . $data_array['block_termexpireaction']
                           . " WHERE block_id = '" . $data_array['block_id'] . "'";
                    $result = $this->db->Execute($sql);
                    $num_result = $this->db->Affected_Rows();
        ...
        if ($result === false) {
            ...
            $this->registry->main_class->clearCache(
                null, $this->registry->sitelang . "|systemadmin|modules|blocks|edit|" . $data_array['block_type'] . "|" . $data_array['block_id']
            );
    ...
    public function block_addrsssite_inbase($array)
    {
        ...
        if ((!isset($array['rss_sitename']) || !isset($array['rss_siteurl'])) || ($data_array['rss_sitename'] == "" || $data_array['rss_siteurl'] == "")) {
        ...
        $sql = "INSERT INTO " . PREFIX . "_rss_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
               . "rss_site_name,rss_site_url) VALUES (" . $sequence_array['sequence_value_string'] . "" . $data_array['rss_sitename'] . "," . $data_array['rss_siteurl']
               . ")";
        $result = $this->db->Execute($sql);
        if ($result === false) {
            $error_message = $this->db->ErrorMsg();
    ...
    public function block_editrsssite_inbase($array)
    {
        ...
        if ((!isset($array['rss_id']) || !isset($array['rss_sitename']) || !isset($array['rss_siteurl']))
            || ($data_array['rss_id'] == 0
                || $data_array['rss_sitename'] == ""
                || $data_array['rss_siteurl'] == "")
        ) {
        ...
        $sql = "UPDATE " . PREFIX . "_rss_" . $this->registry->sitelang . " SET rss_site_name = " . $data_array['rss_sitename'] . ",rss_site_url = "
               . $data_array['rss_siteurl'] . " WHERE rss_id = '" . $data_array['rss_id'] . "'";
        $result = $this->db->Execute($sql);
        $num_result = $this->db->Affected_Rows();
        if ($result === false) {
            $error_message = $this->db->ErrorMsg();
