...
class controller_admin_guestbook extends controller_base
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['pend']) && intval($_GET['pend']) != 0 && intval($_GET['pend']) == 1) {
            $temp = "pending";
        ...
        if (!$this->isCached(
            ...
            if ($error === 'unknown_page' || $error === 'show_' . $temp . '_sql_error') {
    ...
    public function guestbook_status()
    {
        ...
        $error_array = $this->model_admin_guestbook->get_property_value('error_array');
        $result = $this->model_admin_guestbook->get_property_value('result');
        if ($error === 'empty_pend_data' || $error === 'empty_data' || $error === 'unknown_action' || $error === 'status_sql_error' || $error === 'status_ok') {
    ...
    public function guestbook_edit()
    {
        ...
        if (!$this->isCached(
            "systemadmin/main.html", $this->registry->sitelang . "|systemadmin|modules|guestbook|edit" . $temp . "|" . $type . "|" . $this->registry->language
        )
        ) {
    ...
    public function guestbook_edit_inbase()
    {
        ...
        $error_array = $this->model_admin_guestbook->get_property_value('error_array');
        $result = $this->model_admin_guestbook->get_property_value('result');
        if ($error === 'edit_not_all_data' || $error === 'edit_incorrect_email' || $error === 'edit_sql_error' || $error === 'edit_ok') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
