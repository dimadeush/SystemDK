...
class admin_guestbook extends model_base
{
    ...
    public function index($array)
    {
        ...
        if (isset($array['pend']) && intval($array['pend']) != 0 && intval($array['pend']) == 1) {
            $temp = "pending";
            $where = "post_status = '0'";
            $sql_order = "ASC";
        } else {
        ...
        if ($result) {
            ...
            $sql = "SELECT post_id,post_user_name,post_user_email,post_user_id,post_user_ip,post_date,post_status,post_answer_date FROM " . PREFIX
                   . "_guestbook WHERE " . $where . " order by post_date " . $sql_order;
        ...
        if ($result) {
            ...
            if ($row_exist > 0) {
                while (!$result->EOF) {
                    ...
                    $post_date = date(
                        "d.m.Y H:i", $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['5']))
                    );
            ...
            if (defined('SYSTEMDK_GUESTBOOK_CONFIRM_NEED') && SYSTEMDK_GUESTBOOK_CONFIRM_NEED == "yes" && $temp == "current") {
    ...
    public function guestbook_status($array)
    {
        ...
        if ($post_id == 0 && isset($array['post_post_id']) && is_array($array['post_post_id']) && count($array['post_post_id']) > 0) {
            $post_id_is_arr = 1;
        ...
        if (!isset($post_id) || $action == "" || $post_id == "'0'") {
            if ($action == "add" || $action == "deletepend") {
        ...
        if ($action == "on") {
            ...
            $result = $this->db->Execute($sql);
        } elseif ($action == "delete" || $action == "deletepend") {
        ...
        if ($result === false) {
            ...
            if ($action != "off" && $action != "on" && $action != "delete") {
    ...
    public function guestbook_edit($array)
    {
        ...
        if (!isset($array['post_id']) || $post_id == 0) {
            $this->error = 'empty_data';

            return;
        }
        $sql =
            "SELECT post_id,post_user_name,post_user_country,post_user_city,post_user_telephone,post_show_telephone,post_user_email,post_show_email,post_user_id,post_user_ip,post_date,post_text,post_status,post_answer,post_answer_date FROM "
            . PREFIX . "_guestbook WHERE " . $where1 . " and post_id = " . $post_id;
        $result = $this->db->Execute($sql);
        if ($result) {
            if (isset($result->fields['0'])) {
                $row_exist = intval($result->fields['0']);
    ...
    public function guestbook_edit_inbase($array)
    {
        ...
        if ((!isset($array['post_id']) || !isset($array['post_user_name']) || !isset($array['post_user_country']) || !isset($array['post_user_city'])
             || !isset($array['post_user_telephone'])
             || !isset($array['post_user_email'])
             || !isset($array['post_date'])
             || !isset($array['post_hour'])
             || !isset($array['post_minute'])
             || !isset($array['post_text'])
             || !isset($array['post_status'])
             || !isset($array['post_answer']))
            || ($data_array['post_id'] == 0 || $data_array['post_user_name'] == "" || $data_array['post_user_email'] == "" || $data_array['post_date'] == ""
                || $data_array['post_text'] == "")
        ) {
        ...
        if ($data_array['post_answer'] !== "") {
            if ($data_array['post_answer_date'] !== "no") {
                $data_array['post_answer_date'] = $data_array['post_answer_date'] . " " . sprintf("%02d", $data_array['post_answer_hour']) . ":" . sprintf(
                        "%02d", $data_array['post_answer_minute']
                    );
        ...
        if ($check_db_need_lobs == 'yes') {
            $this->db->StartTrans();
            $sql = "UPDATE " . PREFIX . "_guestbook SET post_user_name = " . $data_array['post_user_name'] . ",post_user_country = "
                   . $data_array['post_user_country'] . ",post_user_city = " . $data_array['post_user_city'] . ",post_user_telephone = "
                   ...
                   . $data_array['post_status'] . "',post_answer = empty_clob(),post_answer_date = " . $data_array['post_answer_date'] . " WHERE post_id = '"
                   . $data_array['post_id'] . "'";
            $result = $this->db->Execute($sql);
            $num_result = $this->db->Affected_Rows();
            if ($result === false) {
            ...
            if ($data_array['post_answer'] != 'NULL') {
                ...
                $result3 = $this->db->UpdateClob(
                    PREFIX . '_guestbook', 'post_answer', $data_array['post_answer'], 'post_id = ' . $data_array['post_id']
                );
                if ($result3 === false) {
                    $error_message = $this->db->ErrorMsg();
                    $error_code = $this->db->ErrorNo();
            ...
            $sql = "UPDATE " . PREFIX . "_guestbook SET post_user_name = " . $data_array['post_user_name'] . ",post_user_country = "
                   ...
                   . $data_array['post_user_telephone']
                   . ",post_show_telephone = '" . $data_array['post_show_telephone'] . "',post_user_email = " . $data_array['post_user_email'] . ",post_show_email = '"
                   . $data_array['post_show_email'] . "',post_date = '" . $data_array['post_date'] . "',post_text = " . $data_array['post_text'] . ",post_status = '"
                   . $data_array['post_status'] . "',post_answer = " . $data_array['post_answer'] . ",post_answer_date = " . $data_array['post_answer_date']
                   . " WHERE post_id = '" . $data_array['post_id'] . "'";
            $result = $this->db->Execute($sql);
            $num_result = $this->db->Affected_Rows();
            if ($result === false) {
                $error_message = $this->db->ErrorMsg();
        ...
        if ($result === false) {
            ...
            if (($data_array['pend'] == 1 && $data_array['post_status'] > 0) || $data_array['pend'] == 0) {
                $this->registry->main_class->systemdk_clearcache("systemadmin|modules|guestbook|showcurrent");
                $this->registry->main_class->systemdk_clearcache("modules|guestbook|display");
    ...
    function guestbook_config_save($array)
    {
        ...
        if ((!isset($array['systemdk_guestbook_metakeywords']) || !isset($array['systemdk_guestbook_metadescription']) || !isset($array['systemdk_guestbook_storynum'])
             || !isset($array['systemdk_guestbook_antispam_ipaddr'])
             || !isset($array['systemdk_guestbook_antispam_num'])
             || !isset($array['systemdk_guestbook_confirm_need'])
             || !isset($array['systemdk_guestbook_sortorder']))
            || ($data_array['systemdk_guestbook_storynum'] == 0 || $data_array['systemdk_guestbook_antispam_ipaddr'] == ""
                || $data_array['systemdk_guestbook_antispam_num'] == 0
                || $data_array['systemdk_guestbook_confirm_need'] == ""
                || $data_array['systemdk_guestbook_sortorder'] == "")
        ) {
