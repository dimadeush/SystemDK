...
class admin_questionnaires extends model_base {
    ...
    public function index($array) {
        ...
        if($result) {
            ...
            $sql = "SELECT questionnaire_id,questionnaire_name,questionnaire_date,questionnaire_status FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." ORDER BY questionnaire_date ASC";
    ...
    public function questionnaire_add_inbase($array) {
        ...
        if(empty($data_array['questionnaire_name']) or !isset($data_array['questionnaire_status'])) {
        ...
        $sequence_array = $this->registry->main_class->db_process_sequence(PREFIX."_questionnaires_".$this->registry->sitelang,'questionnaire_id');
        $sql = "INSERT INTO ".PREFIX."_q_questionnaires_".$this->registry->sitelang."(".$sequence_array['field_name_string']."questionnaire_name,questionnaire_date,questionnaire_status,questionnaire_meta_title,questionnaire_meta_keywords,questionnaire_meta_description) VALUES (".$sequence_array['sequence_value_string']."".$data_array['questionnaire_name'].",'".$now."','".$data_array['questionnaire_status']."',".$data_array['questionnaire_meta_title'].",".$data_array['questionnaire_meta_keywords'].",".$data_array['questionnaire_meta_description'].")";
        $result = $this->db->Execute($sql);
        if($result === false) {
            $error_message = $this->db->ErrorMsg();
            $error_code = $this->db->ErrorNo();
    ...
    public function questionnaire_edit($array) {
        ...
        if(empty($array['questionnaire_id']) or $array['questionnaire_id'] < 1) {
            $this->error = 'questionnaire_empty_data';
            return;
        }
        $sql = "SELECT questionnaire_id,questionnaire_name,questionnaire_date,questionnaire_status,questionnaire_meta_title,questionnaire_meta_keywords,questionnaire_meta_description FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." WHERE questionnaire_id = '".$array['questionnaire_id']."'";
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error_message = $this->db->ErrorMsg();
            $error_code = $this->db->ErrorNo();
        ...
        $this->result['questionnaire_all'] = array(
            "questionnaire_id" => $questionnaire_id,
            "questionnaire_name" => $questionnaire_name,
            "questionnaire_date" => $questionnaire_date,
            "questionnaire_status" => $questionnaire_status,
    ...
    public function questionnaire_edit_inbase($array) {
        ...
        if(empty($data_array['questionnaire_name']) or empty($data_array['questionnaire_id']) or !isset($data_array['questionnaire_status'])) {
            $this->error = 'not_all_data';
            return;
        }
        $data_array['questionnaire_name'] = $this->registry->main_class->processing_data($data_array['questionnaire_name']);
        ...
        $sql = "UPDATE ".PREFIX."_q_questionnaires_".$this->registry->sitelang." SET questionnaire_name = ".$data_array['questionnaire_name'].$sql_add.",questionnaire_status = '".$data_array['questionnaire_status']."',questionnaire_meta_title = ".$data_array['questionnaire_meta_title'].",questionnaire_meta_keywords = ".$data_array['questionnaire_meta_keywords'].",questionnaire_meta_description = ".$data_array['questionnaire_meta_description']." WHERE questionnaire_id = '".$data_array['questionnaire_id']."'";
        $result = $this->db->Execute($sql);
        ...
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|show");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|modules|questionnaires|questionnaires_list");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|edit_questionnaire|".$data_array['questionnaire_id']);
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|modules|questionnaires|view|".$data_array['questionnaire_id']);
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|group_add");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|edit_group");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|questions");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question_add");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question_edit");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|results");
        $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|results_details|questionnaire|".$data_array['questionnaire_id']."|group");
    ...
    public function questionnaires_status($array) {
        ...
        if($questionnaire_id == 0 and isset($array['post_questionnaire_id']) and is_array($array['post_questionnaire_id']) and count($array['post_questionnaire_id']) > 0) {
        ...
        if(!isset($questionnaire_id) or $action == "" or $questionnaire_id == "'0'") {
            $this->error = 'empty_data';
            return;
        }
        if($action == "on") {
            $sql = "UPDATE ".PREFIX."_q_questionnaires_".$this->registry->sitelang." SET questionnaire_status = '1' WHERE questionnaire_id in (".$questionnaire_id.")";
            $result = $this->db->Execute($sql);
            if($result) {
                $num_result = $this->db->Affected_Rows();
            }
        } elseif($action == "off") {
            $sql = "UPDATE ".PREFIX."_q_questionnaires_".$this->registry->sitelang." SET questionnaire_status = '0' WHERE questionnaire_id IN (".$questionnaire_id.")";
            $result = $this->db->Execute($sql);
            if($result) {
                $num_result = $this->db->Affected_Rows();
            }
        } elseif($action == "confirm_delete") {
            $sql = "SELECT questionnaire_id,questionnaire_name FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." WHERE questionnaire_id IN (".$questionnaire_id.")";
            $result = $this->db->Execute($sql);
            if(!$result) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
            ...
            $this->db->StartTrans();
            $sql1 = "DELETE FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_id in (SELECT question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id IN (".$questionnaire_id."))";
            $result1 = $this->db->Execute($sql1);
            if($result1 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
            $sql2 = "DELETE FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_id in (SELECT question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id IN (".$questionnaire_id."))";
            ...
            $sql5 = "DELETE FROM ".PREFIX."_q_voted_persons_".$this->registry->sitelang." WHERE voted_group_id IN (SELECT voted_group_id FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." WHERE questionnaire_id IN (".$questionnaire_id."))";
            ...
            if($result1 === false or $result2 === false or $result3 === false or $result4 === false or $result5 === false or $result6 === false or $result7 === false) {
                $result = false;
            }
        ...
        if($questionnaire_id_is_arr == 1) {
            ...
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|edit_questionnaire|".intval($array['get_questionnaire_id']));
            if($action == "delete") {
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".intval($array['get_questionnaire_id'])."|questions");
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".intval($array['get_questionnaire_id'])."|question_add");
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".intval($array['get_questionnaire_id'])."|question");
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".intval($array['get_questionnaire_id'])."|question_edit");
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|results_details|questionnaire|".intval($array['get_questionnaire_id'])."|group");
            }
        }
        $this->result['questionnaires_message'] = $action;
    }
    ...
    public function group_add_inbase($array) {
        ...
        if(empty($data_array['group_name']) or !isset($data_array['group_status']) or empty($data_array['questionnaire_id'])) {
            $this->error = 'not_all_data';
            return;
        }
        ...
        $sql = "INSERT INTO ".PREFIX."_q_groups_".$this->registry->sitelang."(".$sequence_array['field_name_string']."questionnaire_id,group_name,group_status) VALUES (".$sequence_array['sequence_value_string']."'".$data_array['questionnaire_id']."',".$data_array['group_name'].",'".$data_array['group_status']."')";
        $result = $this->db->Execute($sql);
        if($result === false) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
    ...
    private function check_questionnaire_assign($questionnaire_id,$exclude_group_id = false) {
        ...
        $sql = "SELECT count(*) FROM ".PREFIX."_q_groups_".$this->registry->sitelang." WHERE questionnaire_id = '".$questionnaire_id."' and group_status = '1'".$sql_add;
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
    ...
    public function group_edit($array) {
        ...
        if(empty($array['group_id']) or $array['group_id'] < 1) {
            $this->error = 'empty_data';
            return;
        }
        $sql = "SELECT a.group_id,a.questionnaire_id,a.group_name,a.group_status,null,null FROM ".PREFIX."_q_groups_".$this->registry->sitelang." a WHERE a.group_id = '".$array['group_id']."' UNION ALL SELECT null,null,null,null,b.questionnaire_id,b.questionnaire_name FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." b";
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
        ...
        if($row_exist1 < 1 and $row_exist2 < 1) {
            if($row_exist1 < 1) {
        ...
        while(!$result->EOF) {
            if(empty($this->result['group_all']) and intval($result->fields['0']) > 0) {
                $group_id = intval($result->fields['0']);
                $questionnaire_id = intval($result->fields['1']);
                $group_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['2']));
                $group_status = intval($result->fields['3']);
                $this->result['group_all'] = array(
                    "group_id" => $group_id,
                    "questionnaire_id" => $questionnaire_id,
                    "group_name" => $group_name,
                    "group_status" => $group_status,
            ...
            if(isset($result->fields['4']) and intval($result->fields['4']) > 0) {
                ...
                $questionnaire_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['5']));
                $this->result['questionnaires_all'][] = array(
                    "questionnaire_id" => $questionnaire_id,
                    "questionnaire_name" => $questionnaire_name,
                );
            }
    ...
    public function group_edit_inbase($array) {
        ...
        if(empty($data_array['group_name']) or empty($data_array['group_id']) or empty($data_array['questionnaire_id']) or !isset($data_array['group_status'])) {
            $this->error = 'not_all_data';
            return;
        }
        ...
        $data_array['group_name'] = $this->registry->main_class->processing_data($data_array['group_name']);
        $sql = "UPDATE ".PREFIX."_q_groups_".$this->registry->sitelang." SET questionnaire_id = '".$data_array['questionnaire_id']."',group_name = ".$data_array['group_name'].",group_status = '".$data_array['group_status']."' WHERE group_id = '".$data_array['group_id']."'";
        $result = $this->db->Execute($sql);
        $num_result = $this->db->Affected_Rows();
        if($result === false) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
    ...
    public function groups_status($array) {
        ...
        if($group_id == 0 and isset($array['post_group_id']) and is_array($array['post_group_id']) and count($array['post_group_id']) > 0) {
            $group_id_is_arr = 1;
        ...
        $action = $this->registry->main_class->format_striptags($action);
        if(!isset($group_id) or $action == "" or $group_id == "'0'") {
            $this->error = 'empty_data';
            return;
        }
        if($action == "group_on") {
            // TODO: Currently group activation only for one POST or GET group_id
            $sql = "SELECT count(a.group_id) FROM ".PREFIX."_q_groups_".$this->registry->sitelang." a WHERE a.questionnaire_id in (SELECT b.questionnaire_id FROM ".PREFIX."_q_groups_".$this->registry->sitelang." b WHERE b.group_id = ".$group_id.") and a.group_status='1' and a.group_id <> ".$group_id."";
            $result = $this->db->Execute($sql);
            if($result) {
                if(isset($result->fields['0'])) {
                    $row_exist = intval($result->fields['0']);
            ...
            $this->db->StartTrans();
            $sql1 = "DELETE FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_id IN (SELECT question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id IN (SELECT questionnaire_id FROM ".PREFIX."_q_groups_".$this->registry->sitelang." WHERE group_id IN (".$group_id.")))";
            $result1 = $this->db->Execute($sql1);
            if($result1 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
            $sql2 = "DELETE FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_id IN (SELECT question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id IN (SELECT questionnaire_id FROM ".PREFIX."_q_groups_".$this->registry->sitelang." WHERE group_id IN (".$group_id.")))";
            $result2 = $this->db->Execute($sql2);
            if($result2 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
            $sql3 = "DELETE FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id IN (SELECT questionnaire_id FROM ".PREFIX."_q_groups_".$this->registry->sitelang." WHERE group_id IN (".$group_id."))";
            $result3 = $this->db->Execute($sql3);
            if($result3 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
            ...
            $sql5 = "DELETE FROM ".PREFIX."_q_voted_persons_".$this->registry->sitelang." WHERE voted_group_id IN (SELECT voted_group_id FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." WHERE group_id IN (".$group_id."))";
            $result5 = $this->db->Execute($sql5);
            if($result5 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
            ...
            $sql7 = "DELETE FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." WHERE questionnaire_id IN (SELECT questionnaire_id FROM ".PREFIX."_q_groups_".$this->registry->sitelang." WHERE group_id IN (".$group_id."))";
            $result7 = $this->db->Execute($sql7);
            if($result7 === false) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            }
        ...
        if(in_array($action,array('group_on','group_off'))) {
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|modules|questionnaires|questionnaires_list");
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|modules|questionnaires|view");
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|groups");
            if($group_id_is_arr == 1) {
                ...
                $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|edit_group|".intval($array['get_group_id']));
            }
        } else {
    ...
    private function get_questions($questionnaire_id,$group_id = false,$questionnaire_results = false) {
        ...
        if($questionnaire_results) {
            ...
            if($questionnaire_results === 'results_by_persons') {
                $fields_add2 = ",l.person_id,m.question_item_id as answer_question_item_id,(CASE WHEN n.question_answer_id IS NOT NULL THEN d.question_answer ELSE NULL END) as answer_question_answer";
                ...
                $join_tables = "LEFT OUTER JOIN ".PREFIX."_q_voted_persons_".$this->registry->sitelang." l on l.voted_group_id IN (SELECT l2.voted_group_id FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." l2 WHERE l2.group_id = '".$group_id."' and l2.questionnaire_id = a.questionnaire_id)
                LEFT OUTER JOIN ".PREFIX."_q_voted_answers_".$this->registry->sitelang." m on a.questionnaire_id = m.questionnaire_id and m.group_id = '".$group_id."' and b.question_id = m.question_id and b.type_id in (1,2,4,5)  and e.question_item_id = m.question_item_id and l.person_id = m.person_id
                LEFT OUTER JOIN ".PREFIX."_q_voted_answers_".$this->registry->sitelang." n on a.questionnaire_id = n.questionnaire_id and n.group_id = '".$group_id."' and b.question_id = n.question_id and b.type_id in (3,6) and e.question_item_id = n.question_item_id and d.question_answer_id = n.question_answer_id and l.person_id = n.person_id
                LEFT OUTER JOIN ".PREFIX."_q_voted_answers_".$this->registry->sitelang." o on a.questionnaire_id = o.questionnaire_id and o.group_id = '".$group_id."' and b.question_id = o.question_id and b.type_id in (4,5,6,7) and l.person_id = o.person_id ";
            }
            $tables_add = PREFIX."_q_voted_groups_".$this->registry->sitelang." f, ".PREFIX."_q_groups_".$this->registry->sitelang." g, ";
            $join_tables = $join_tables."LEFT OUTER JOIN ".PREFIX."_q_voted_answers_".$this->registry->sitelang." h on a.questionnaire_id = h.questionnaire_id and h.group_id = '".$group_id."' and b.question_id = h.question_id and b.type_id in (1,2,4,5)  and e.question_item_id = h.question_item_id
            LEFT OUTER JOIN ".PREFIX."_q_voted_answers_".$this->registry->sitelang." k on a.questionnaire_id = k.questionnaire_id and k.group_id = '".$group_id."' and b.question_id = k.question_id and b.type_id in (3,6)  and e.question_item_id = k.question_item_id and d.question_answer_id = k.question_answer_id";
        ...
        $sql = "SELECT ".$fields.$fields_add.$count_fields.$fields_add2."
                ...
                LEFT OUTER JOIN ".PREFIX."_q_question_items_".$this->registry->sitelang." e on b.question_id=e.question_id ".$join_tables." WHERE ".$where_add."a.questionnaire_id = ".$questionnaire_id.$group_by." ORDER BY b.question_priority,d.question_answer_priority,e.question_item_priority ASC";
        //echo $sql; exit();
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
        ...
        while(!$result->EOF) {
            ...
            if($questionnaire_results and $questionnaire_results === 'results_by_persons') {
            ...
            if(!isset($results['questions'][$question_id])) {
                ...
                if($this->check_need_question_subtext($type_id)) {
                    $results['questions'][$question_id] = array_merge($results['questions'][$question_id],array('question_subtext' => $question_subtext));
                }
            }
            if($questionnaire_results and $questionnaire_results === 'results_by_persons' and $question_item_id < 1) {
                $results['questions'][$question_id]['results_by_persons'][$person_id] = array(
                    'person_id' => $person_id,
            ...
            if($question_answer_id < 1) {
                ...
                $question_answer = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['13']));
                ...
                if($questionnaire_results) {
                    if($questionnaire_results === 'results_by_persons') {
                        if($answer_question_answer !== '') {
                            $results['questions'][$question_id]['question_items'][$question_item_id]['results_by_persons'][$person_id]['answer_question_answer'] = $answer_question_answer;
                        }
                    } else {
                        $results['questions'][$question_id]['question_items'][$question_item_id]['count_voted_question_answers'][$question_answer_id] = intval($result->fields['20']);
                    }
                }
            }
            if($question_item_id < 1) {
                ...
                $question_item_title = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['16']));
                ...
                if($questionnaire_results) {
                    ...
                    if($questionnaire_results === 'results_by_persons') {
                        ...
                        $results['questions'][$question_id]['question_items'][$question_item_id]['results_by_persons'][$person_id]['answer_question_item_id'] = $answer_question_item_id;
                        $results['questions'][$question_id]['results_by_persons'][$person_id]['answer_text'] = $answer_text; //['question_items'][$question_item_id]
                    }
                }
            }
            $result->MoveNext();
    ...
    private function check_allow_question_items($question_type_id) {
        if(in_array($question_type_id,array(1,2,3,4,5,6))) {
            return true;
        }
        return false;
    }
    ...
    private function check_allow_answer_variants($question_type_id) {
        if(in_array($question_type_id,array(3,6))) {
            return true;
        }
        return false;
    }
    ...
    private function check_need_question_subtext($question_type) {
        if(in_array($question_type,array(4,5,6,7))) {
            return true;
        }
        return false;
    }
    ...
    public function question_add($questionnaire_id) {
        ...
        $sql = "SELECT questionnaire_id,questionnaire_name,null as type_id,null as type_name,null as type_description FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." WHERE questionnaire_id = ".$questionnaire_id." UNION ALL SELECT null as questionnaire_id,null as questionnaire_name,type_id,type_name,type_description FROM ".PREFIX."_q_question_types";
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
        ...
        while(!$result->EOF) {
            if(empty($this->result['questionnaires_all']) and intval($result->fields['0']) > 0) {
                $questionnaire_id = intval($result->fields['0']);
                $questionnaire_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['1']));
                $this->result['questionnaires_all'] = array(
                    "questionnaire_id" => $questionnaire_id,
            ...
            if(isset($result->fields['2']) and intval($result->fields['2']) > 0) {
                $type_id = intval($result->fields['2']);
                $type_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['3']));
                $type_description = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['4']));
                $this->result['question_types_all'][] = array(
                    "type_id" => $type_id,
                    "type_name" => $type_name,
                    "type_description" => $type_description,
                );
    ...
    public function process_questionnaire_and_question($array) {
        ...
        $sql = "SELECT a.questionnaire_id,a.questionnaire_name,b.question_id,b.question_title,b.question_subtext,b.question_obligatory,b.type_id FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." a LEFT OUTER JOIN ".PREFIX."_q_questions_".$this->registry->sitelang." b ON (a.questionnaire_id=b.questionnaire_id and b.question_id = ".$data_array['question_id'].") WHERE a.questionnaire_id = ".$data_array['questionnaire_id'];
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
    ...
    public function question_add_inbase($array) {
        ...
        if(empty($data_array['questionnaire_id']) or empty($data_array['title']) or empty($data_array['question_type']) or !isset($data_array['question_obligatory']) or ($this->check_need_question_subtext($data_array['question_type']) and empty($data_array['question_subtext']))
        ) {
            $this->error = 'not_all_data';
            return;
        }
        ...
        $sql = "SELECT max(question_priority) FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE questionnaire_id = ".$data_array['questionnaire_id'];
        $result = $this->db->Execute($sql);
        if($result) {
            if(isset($result->fields['0'])) {
        ...
        if($result) {
            ...
            $sql = "INSERT INTO ".PREFIX."_q_questions_".$this->registry->sitelang."(".$sequence_array['field_name_string']."question_title,questionnaire_id,type_id,question_priority,question_subtext,question_obligatory) VALUES (".$sequence_array['sequence_value_string']."".$data_array['title'].",".$data_array['questionnaire_id'].",".$data_array['question_type'].",'".$question_priority."',".$data_array['question_subtext'].",'".$data_array['question_obligatory']."')";
            $result = $this->db->Execute($sql);
        }
        if($result === false) {
            $error_message = $this->db->ErrorMsg();
    ...
    public function item_or_answer_add_inbase($array) {
        ...
        if(empty($data_array['questionnaire_id']) or empty($data_array['question_id']) or empty($data_array['title']) or empty($data_array['questionnaire_add_type'])) {
            $this->error = 'not_all_data';
            return;
        }
        $data_array['title'] = $this->registry->main_class->processing_data($data_array['title']);
        if($data_array['questionnaire_add_type'] === 'question_item_add') {
            $sql = "SELECT max(question_item_priority) FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_id = ".$data_array['question_id'];
        } else {
            $sql = "SELECT max(question_answer_priority) FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_id = ".$data_array['question_id'];
        }
        $result = $this->db->Execute($sql);
        if($result) {
            if(isset($result->fields['0'])) {
        ...
        if($result) {
            if($data_array['questionnaire_add_type'] === 'question_item_add') {
                ...
                $sql = "INSERT INTO ".PREFIX."_q_question_items_".$this->registry->sitelang."(".$sequence_array['field_name_string']."question_id,question_item_title,question_item_priority) VALUES (".$sequence_array['sequence_value_string']."".$data_array['question_id'].",".$data_array['title'].",".$priority.")";
            } else {
                ...
                $sql = "INSERT INTO ".PREFIX."_q_question_answers_".$this->registry->sitelang."(".$sequence_array['field_name_string']."question_id,question_answer,question_answer_priority) VALUES (".$sequence_array['sequence_value_string']."".$data_array['question_id'].",".$data_array['title'].",".$priority.")";
            }
            $result = $this->db->Execute($sql);
        }
        if($result === false) {
    ...
    public function get_question_item_or_answer($array) {
        ...
        if($array['questionnaire_action'] == 'question_item_edit' and empty($data_array[$item])) {
            $this->error = 'empty_question_item';
            return;
        }
        if($array['questionnaire_action'] == 'question_answer_edit' and empty($data_array[$item])) {
        ...
        $sql = "SELECT a.questionnaire_id,a.questionnaire_name,b.question_id,b.question_title,".$fields." FROM ".PREFIX."_q_questionnaires_".$this->registry->sitelang." a LEFT OUTER JOIN ".PREFIX."_q_questions_".$this->registry->sitelang." b ON (a.questionnaire_id=b.questionnaire_id and b.question_id = ".$data_array['question_id'].") LEFT OUTER JOIN ".PREFIX."_".$table."_".$this->registry->sitelang." c ON (".$clause.") WHERE a.questionnaire_id = ".$data_array['questionnaire_id'];
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
    ...
    public function question_or_item_or_answer_edit_inbase($array) {
        ...
        if($data_array['questionnaire_action'] == 'question_item_edit' and empty($data_array[$item])) {
            $this->error = 'empty_question_item';
            return;
        }
        if($data_array['questionnaire_action'] == 'question_answer_edit' and empty($data_array[$item])) {
            $this->error = 'empty_question_answer';
            return;
        }
        if(empty($data_array['title']) or ($data_array['questionnaire_action'] == 'question_edit' and (empty($data_array['question_type']) or !isset($data_array['question_obligatory']) or ($this->check_need_question_subtext($data_array['question_type']) and empty($data_array['question_subtext']))))) {
            $this->error = 'not_all_data';
            return;
        }
        $data_array['title'] = $this->registry->main_class->processing_data($data_array['title']);
        ...
        if($data_array['questionnaire_action'] == 'question_answer_edit') {
            ...
            $fields = "question_title = ".$data_array['title'].", question_subtext = ".$data_array['question_subtext'].", question_obligatory = '".$data_array['question_obligatory']."'";
            $table = "q_questions";
        ...
        if($data_array['questionnaire_action'] == 'question_edit') {
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question_edit|".$data_array['question_id']);
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question|".$data_array['question_id']."|question_item_edit");
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question|".$data_array['question_id']."|question_answer_edit");
        } else {
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question|".$data_array['question_id']."|".$data_array['questionnaire_action']."|".$data_array[$item]);
        }
        $this->result['questionnaire_id'] = $data_array['questionnaire_id'];
    ...
    public function question_or_item_or_answer_delete($array) {
        ...
        if(empty($data_array['type']) or empty($data_array['action']) or !in_array($data_array['type'],array(
                'question_delete',
                'question_item_delete',
                'question_answer_delete'
            )) or !in_array($data_array['action'],array('confirm_delete','delete'))
        ) {
            $this->error = 'empty_data';
            return;
        }
        if($data_array['action'] != "delete") {
            if($data_array['type'] == 'question_delete') {
                ...
                $sql = "SELECT question_id,question_title,questionnaire_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."'";
                ...
                $sql = "SELECT a.question_item_id,a.question_item_title,b.questionnaire_id FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." a,".PREFIX."_q_questions_".$this->registry->sitelang." b WHERE a.question_id = b.question_id and a.question_item_id = '".$data_array['question_item_id']."'";
                ...
                $sql = "SELECT a.question_answer_id,a.question_answer,b.questionnaire_id FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." a,".PREFIX."_q_questions_".$this->registry->sitelang." b WHERE a.question_id = b.question_id and a.question_answer_id = '".$data_array['question_answer_id']."'";
            }
            $result = $this->db->Execute($sql);
            if(!$result) {
                $error_message = $this->db->ErrorMsg();
            ...
            $delete_title = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['1']));
        ...
        if($data_array['type'] == 'question_delete') {
            ...
            $sql_for_priority = "SELECT question_id,question_priority FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."' and questionnaire_id = '".$data_array['questionnaire_id']."'";
            ...
            $sql1 = "UPDATE ".PREFIX."_q_questions_".$this->registry->sitelang." SET question_priority = question_priority-1 WHERE questionnaire_id = '".$data_array['questionnaire_id']."' and question_priority > '".$question_priority."'";
            $sql2 = "DELETE FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." WHERE question_item_id IN (SELECT a.question_item_id FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." a,".PREFIX."_q_questions_".$this->registry->sitelang." b WHERE a.question_id = b.question_id and a.question_id = '".$data_array['question_id']."' and b.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $sql3 = "DELETE FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."' and questionnaire_id = '".$data_array['questionnaire_id']."'";
            $sql4 = "DELETE FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.question_id = '".$data_array['question_id']."' and a.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $sql5 = "DELETE FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.question_id = '".$data_array['question_id']."' and a.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $sql = "DELETE FROM ".PREFIX."_q_questions_".$this->registry->sitelang." WHERE question_id = '".$data_array['question_id']."' and questionnaire_id = '".$data_array['questionnaire_id']."'";
        } elseif($data_array['type'] == 'question_item_delete') {
            if(empty($data_array['question_item_id'])) {
                $this->error = 'empty_question_item';
                return;
            }
            $sql_for_priority = "SELECT question_id,question_item_priority FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_item_id = '".$data_array['question_item_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $result_for_priority = $this->db->Execute($sql_for_priority);
            if(!$result_for_priority) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
                $this->error = 'sql_error';
            ...
            $sql1 = "UPDATE ".PREFIX."_q_question_items_".$this->registry->sitelang." SET question_item_priority = question_item_priority-1 WHERE question_id = '".$question_id."' and question_item_priority > '".$question_item_priority."'";
            $sql2 = "DELETE FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." WHERE question_item_id IN (SELECT a.question_item_id FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." a,".PREFIX."_q_questions_".$this->registry->sitelang." b WHERE a.question_id = b.question_id and a.question_item_id = '".$data_array['question_item_id']."' and b.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $sql = "DELETE FROM ".PREFIX."_q_question_items_".$this->registry->sitelang." WHERE question_item_id = '".$data_array['question_item_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.questionnaire_id = '".$data_array['questionnaire_id']."')";
        } elseif($data_array['type'] == 'question_answer_delete') {
            if(empty($data_array['question_answer_id'])) {
                $this->error = 'empty_question_answer';
                return;
            }
            $sql_for_priority = "SELECT question_id,question_answer_priority FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_answer_id = '".$data_array['question_answer_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $result_for_priority = $this->db->Execute($sql_for_priority);
            if(!$result_for_priority) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
                $this->error = 'sql_error';
            ...
            $sql1 = "UPDATE ".PREFIX."_q_question_answers_".$this->registry->sitelang." SET question_answer_priority = question_answer_priority-1 WHERE question_id = '".$question_id."' and question_answer_priority > '".$question_answer_priority."'";
            $sql2 = "DELETE FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." WHERE question_answer_id IN (SELECT a.question_answer_id FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." a,".PREFIX."_q_questions_".$this->registry->sitelang." b WHERE a.question_id = b.question_id and a.question_answer_id = '".$data_array['question_answer_id']."' and b.questionnaire_id = '".$data_array['questionnaire_id']."')";
            $sql = "DELETE FROM ".PREFIX."_q_question_answers_".$this->registry->sitelang." WHERE question_answer_id = '".$data_array['question_answer_id']."' and question_id IN (SELECT a.question_id FROM ".PREFIX."_q_questions_".$this->registry->sitelang." a WHERE a.questionnaire_id = '".$data_array['questionnaire_id']."')";
        }
        $this->db->StartTrans();
        $result1 = $this->db->Execute($sql1);
        if($result1 === false) {
        ...
        if($data_array['type'] == 'question_delete') {
            ...
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question|".$question_id."|question_item_edit|".$data_array['question_item_id']);
        } elseif($data_array['type'] == 'question_answer_delete') {
            $this->registry->main_class->clearCache(null,$this->registry->sitelang."|systemadmin|modules|questionnaires|questionnaire|".$data_array['questionnaire_id']."|question|".$question_id."|question_answer_edit|".$data_array['question_answer_id']);
        }
        $this->result['questionnaire_id'] = $data_array['questionnaire_id'];
    ...
    public function results($array) {
        ...
        if($result) {
            $num_rows = intval($result->fields['0']);
            $sql = "SELECT a.voted_group_id,a.group_id,b.group_name,a.questionnaire_id,c.questionnaire_name FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." a,".PREFIX."_q_groups_".$this->registry->sitelang." b,".PREFIX."_q_questionnaires_".$this->registry->sitelang." c  WHERE a.group_id = b.group_id and a.questionnaire_id = c.questionnaire_id ORDER BY a.voted_group_id ASC";
            $result = $this->db->SelectLimit($sql,$num_string_rows,$offset);
        }
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
    ...
    public function results_details($array) {
        ...
        if(empty($data_array['group_id']) or empty($data_array['questionnaire_id'])) {
            $this->error = 'empty_data';
            return;
        }
        $sql = "SELECT count(*) as voted_persons FROM ".PREFIX."_q_voted_persons_".$this->registry->sitelang." WHERE voted_group_id IN (SELECT voted_group_id FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." WHERE group_id = '".$data_array['group_id']."' and questionnaire_id = '".$data_array['questionnaire_id']."')";
        $result = $this->db->Execute($sql);
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
            $this->error = 'sql_error';
    ...
    public function results_person_text_answers($array) {
        ...
        $sql = "SELECT count(*) FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." a,".PREFIX."_q_groups_".$this->registry->sitelang." b,".PREFIX."_q_questionnaires_".$this->registry->sitelang." c,".PREFIX."_q_questions_".$this->registry->sitelang." d  WHERE a.group_id = b.group_id and a.questionnaire_id = c.questionnaire_id and a.question_id = d.question_id and a.group_id = '".$data_array['group_id']."' and a.questionnaire_id = '".$data_array['questionnaire_id']."' and a.question_id = '".$data_array['question_id']."' ".$sql_add." and a.answer_text is not NULL";
        $result = $this->db->Execute($sql);
        if($result) {
            $num_rows = intval($result->fields['0']);
            $sql = "SELECT a.answer_id,a.answer_text,b.group_id,b.group_name,c.questionnaire_id,c.questionnaire_name,d.question_id,d.question_title FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." a,".PREFIX."_q_groups_".$this->registry->sitelang." b,".PREFIX."_q_questionnaires_".$this->registry->sitelang." c,".PREFIX."_q_questions_".$this->registry->sitelang." d  WHERE a.group_id = b.group_id and a.questionnaire_id = c.questionnaire_id and a.question_id = d.question_id and a.group_id = '".$data_array['group_id']."' and a.questionnaire_id = '".$data_array['questionnaire_id']."' and a.question_id = '".$data_array['question_id']."' ".$sql_add." and a.answer_text is not NULL ORDER BY a.answer_id ASC";
            $result = $this->db->SelectLimit($sql,$num_string_rows,$offset);
        }
        if(!$result) {
            $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
        ...
        while(!$result->EOF) {
            ...
            if(!isset($person_text_answers['person_text_answers_data'])) {
                ...
                $group_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['3']));
                $questionnaire_id = intval($result->fields['4']);
                $questionnaire_name = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['5']));
                $question_id = intval($result->fields['6']);
                $question_title = $this->registry->main_class->format_htmlspecchars($this->registry->main_class->extracting_data($result->fields['7']));
                $person_text_answers['person_text_answers_data'] = array(
                    'group_id' => $group_id,
                    'group_name' => $group_name,
                    'questionnaire_id' => $questionnaire_id,
                    'questionnaire_name' => $questionnaire_name,
            ...
            if(!empty($answer_text)) { // workaround for Oracle when empty clob
                $person_text_answers['person_text_answers'][] = array(
                    'answer_id' => $answer_id,
                    'answer_text' => $answer_text,
                );
            }
    ...
    public function results_delete($array) {
        ...
        if($voted_group_id == 0 and isset($array['post_voted_group_id']) and is_array($array['post_voted_group_id']) and count($array['post_voted_group_id']) > 0) {
        ...
        if(!isset($voted_group_id) or $action == "" or $voted_group_id == "'0'" or empty($type)) {
            $this->error = 'empty_data';
            return;
        }
        if($action == "confirm_delete") {
            $sql = "SELECT a.voted_group_id,b.group_name,c.questionnaire_name FROM ".PREFIX."_q_voted_groups_".$this->registry->sitelang." a,".PREFIX."_q_groups_".$this->registry->sitelang." b,".PREFIX."_q_questionnaires_".$this->registry->sitelang." c WHERE a.group_id = b.group_id and a.questionnaire_id = c.questionnaire_id and a.voted_group_id IN (".$voted_group_id.")";
            $result = $this->db->Execute($sql);
            if(!$result) {
                $error[] = array("code" => $this->db->ErrorNo(),"message" => $this->db->ErrorMsg());
                $this->error = 'sql_error';
            ...
            $this->db->StartTrans();
            $sql1 = "DELETE FROM ".PREFIX."_q_voted_answers_".$this->registry->sitelang." WHERE person_id IN (SELECT person_id FROM ".PREFIX."_q_voted_persons_".$this->registry->sitelang." WHERE voted_group_id IN (".$voted_group_id."))";
    ...
    public function config_save($array) {
        ...
        foreach($keys as $key => $value) {
            ...
            if(!empty($array[$key])) {
                $data_array[$key] = $value === 'string' ? $this->registry->main_class->format_htmlspecchars_stripslashes_striptags(trim($array[$key])) : intval($array[$key]);
            }
        }
        if(!isset($data_array['systemdk_questionnaires_organization_id']) or !isset($data_array['systemdk_questionnaires_organization_name']) or !isset($data_array['systemdk_questionnaires_voted_method_check']) or !isset($data_array['systemdk_questionnaires_voted_control_live_time']) or !isset($data_array['systemdk_questionnaires_who_can_vote'])) {
            $this->error = 'not_all_data';
            return;
        }
