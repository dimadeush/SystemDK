...
class controller_admin_questionnaires extends controller_base
{
    ...
    public function index()
    {
        ...
        if (!$this->isCached(
            ...
            if ($error === 'unknown_page' || $error === 'sql_error') {
    ...
    public function questionnaire_edit()
    {
        ...
        if (!$this->isCached(
            ...
            $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $data_array['questionnaire_id'] . "|" . $this->registry->language
        )
        ) {
            ...
            $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
            $result = $this->model_admin_questionnaires->get_property_value('result');
            if ($error === 'questionnaire_empty_data' || $error === 'sql_error' || $error === 'questionnaire_not_found') {
    ...
    public function questionnaire_edit_inbase()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'sql_error' || $error === 'ok') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    public function questionnaires_status()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if ($error === 'empty_data' || $error === 'unknown_action' || $error === 'sql_error' || $error === 'ok') {
    ...
    public function group_add()
    {
        ...
        if (!$this->isCached("systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $type . "|" . $this->registry->language)) {
            ...
            $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
            $result = $this->model_admin_questionnaires->get_property_value('result');
            if ($error === 'no_questionnaires' || $error === 'sql_error') {
                if (!empty($error_array)) {
                    $this->assign("error", $error_array);
                }
                $cache_category = false;
    ...
    public function group_add_inbase()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'sql_error' || $error === 'already_assigned') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    public function group_edit()
    {
        ...
        if (!$this->isCached(
            "systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $data_array['group_id'] . "|" . $this->registry->language
        )
        ) {
            ...
            $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
            $result = $this->model_admin_questionnaires->get_property_value('result');
            if ($error === 'empty_data' || $error === 'sql_error' || $error === 'group_edit_not_found' || $error === 'no_questionnaires') {
                if ($error === 'empty_data') {
                    header("Location: index.php?path=admin_questionnaires&func=groups&lang=" . $this->registry->sitelang);
                    exit();
                }
    ...
    public function group_edit_inbase()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'sql_error' || $error === 'already_assigned' || $error === 'ok') {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    public function groups_status()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if ($error === 'empty_data' || $error === 'already_assigned' || $error === 'unknown_action' || $error === 'sql_error' || $error === 'ok') {
            if ($error === 'empty_data' || $error === 'unknown_action') {
                header("Location: index.php?path=admin_questionnaires&func=groups&lang=" . $this->registry->sitelang);
                exit();
            }
    ...
    public function questions()
    {
        ...
        if (!$this->isCached("systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $type . "|" . $this->registry->language)) {
            ...
            $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
            $result = $this->model_admin_questionnaires->get_property_value('result');
            if (in_array($error, ['empty_data', 'sql_error', 'questionnaire_not_found', 'no_questions'])) {
                if ($error === 'empty_data') {
                    header("Location: index.php?path=admin_questionnaires&func=index&lang=" . $this->registry->sitelang);
                    exit();
                }
    ...
    public function question_add_inbase()
    {
        ...
        $error_array = $this->model_admin_questionnaires->get_property_value('error_array');
        $result = $this->model_admin_questionnaires->get_property_value('result');
        if (in_array($error, ['not_all_data', 'sql_error'])) {
            if (!empty($error_array)) {
                $this->assign("error", $error_array);
            }
            $cache_category = false;
    ...
    private function get_question_item_or_answer($type, $title)
    {
        ...
        if (!$this->isCached(
            "systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $data_array[$item] . "|" . $this->registry->language
        )
        ) {
            ...
            if (!empty($error)) {
                if ($error === 'unknown_questionnaire_action') {
                    header(
                        "Location: index.php?path=admin_questionnaires&func=questions&questionnaire_id=" . $data_array['questionnaire_id'] . "&lang="
    ...
    public function results_details()
    {
        ...
        $keys = ['group_id', 'questionnaire_id'];
        foreach ($keys as $key) {
            $data_array[$key] = 0;
            if (!empty($_GET[$key])) {
    ...
    public function results_person_text_answers()
    {
        ...
        $keys = ['group_id', 'questionnaire_id', 'question_id', 'person_id', 'num_page'];
        foreach ($keys as $key) {
            $data_array[$key] = 0;
            if (!empty($_GET[$key])) {
                $data_array[$key] = intval($_GET[$key]);
        ...
        if (empty($data_array['num_page']) || $data_array['num_page'] < 1) {
            $data_array['num_page'] = 1;
        }
        $title = '_QUESTIONNAIRES_QUESTIONNAIRE_RESULTS_TITLE';
        $cache_category = 'modules|questionnaires|results_details|questionnaire|' . $data_array['questionnaire_id'] . '|group|' . $data_array['group_id'] . '|' . $type
                          . '|' . $data_array['question_id'];
        if ($data_array['person_id'] > 0) {
            $cache_category = 'modules|questionnaires|results_details|questionnaire|' . $data_array['questionnaire_id'] . '|group|' . $data_array['group_id'] . '|person|'
                              . $data_array['person_id'] . '|' . $type . '|' . $data_array['question_id'];
        }
        if (!$this->isCached(
            "systemadmin/main.html", $this->registry->sitelang . "|systemadmin|" . $cache_category . "|" . $data_array['num_page'] . "|" . $this->registry->language
        )
            ...
            if (!empty($error)) {
                if ($error === 'unknown_page') {
                    header(
                        "Location: index.php?path=admin_questionnaires&func=results_details&group_id=" . $data_array['group_id'] . "&questionnaire_id="
                        . $data_array['questionnaire_id'] . "&lang=" . $this->registry->sitelang
                    );
                    exit();
                }
                if (!empty($error_array)) {
    ...
    public function results_delete()
    {
        ...
        $source_array = ['get' => $_GET, 'post' => $_POST];
