...
class controller_admin_modules extends controller_base {
    ...
    public function edit_module() {
        ...
        $template = false;
        if(!$this->isCached("systemadmin/main.html",$this->registry->sitelang."|systemadmin|".$cache_category."|".$module_id."|".$this->registry->language)) {
        ...
        $result = $this->model_admin_modules->get_property_value('result');
        $this->assign_array($result);
        if($error === 'empty_data' or $error === 'edit_sql_error' or $error === 'edit_not_found' or $error === 'show_sql_error') {
            if($error === 'empty_data') {
    ...
    public function edit_module_save() {
        ...
        $result = $this->model_admin_modules->get_property_value('result');
        $this->assign_array($result);
        if($error === 'edit_not_all_data' or $error === 'edit_found' or $error === 'edit_sql_error' or $error === 'edit_ok' or $error === 'show_sql_error') {
            if(!empty($error_array)) {
                $this->assign("error",$error_array);
            }
            if($error === 'show_sql_error') {
    ...
    public function status_module() {
        ...
        $result = $this->model_admin_modules->get_property_value('result');
        $this->assign_array($result);
        if($error === 'empty_data' or $error === 'unknown_action' or $error === 'status_sql_error' or $error === 'status_ok' or $error === 'show_sql_error') {
