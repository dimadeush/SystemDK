...
class controller_admin_menu extends controller_base
{
    ...
    public function menu_save()
    {
        ...
        $error_array = $this->model_admin_menu->get_property_value('error_array');
        $result = $this->model_admin_menu->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'show_sql_error' || $error === 'save_ok') {
    ...
    public function menu_add()
    {
        ...
        $error_array = $this->model_admin_menu->get_property_value('error_array');
        $result = $this->model_admin_menu->get_property_value('result');
        if ($error === 'not_all_data' || $error === 'add_sql_error' || $error === 'add_no_modules' || $error === 'add_no_pages' || $error === 'unknown_menu_type') {
            if ($error === 'not_all_data' || $error === 'unknown_menu_type') {
                header("Location: index.php?path=admin_menu&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
            if (!empty($error_array)) {
    ...
    public function menu_add_inbase()
    {
        ...
        $error_array = $this->model_admin_menu->get_property_value('error_array');
        $result = $this->model_admin_menu->get_property_value('result');
        if ($error === 'add_not_all_data' || $error === 'add_sql_error' || $error === 'unknown_menu_object') {
    ...
    public function menu_edit()
    {
        ...
        $error_array = $this->model_admin_menu->get_property_value('error_array');
        $result = $this->model_admin_menu->get_property_value('result');
        if ($error === 'edit_not_all_data' || $error === 'edit_not_found' || $error === 'edit_sql_error' || $error === 'edit_no_modules' || $error === 'edit_no_pages'
            || $error === 'unknown_menu_type'
        ) {
            if ($error === 'edit_not_all_data' || $error === 'unknown_menu_type') {
                header("Location: index.php?path=admin_menu&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
            if (!empty($error_array)) {
    ...
    public function menu_edit_inbase()
    {
        ...
        $error_array = $this->model_admin_menu->get_property_value('error_array');
        $result = $this->model_admin_menu->get_property_value('result');
        if ($error === 'edit_not_all_data' || $error === 'unknown_menu_object' || $error === 'edit_sql_error' || $error === 'edit_ok') {
            if ($error === 'unknown_menu_object') {
                header("Location: index.php?path=admin_menu&func=index&lang=" . $this->registry->sitelang);
                exit();
            }
