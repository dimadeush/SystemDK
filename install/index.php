...
class install {
    ...
    public function step4() {
        ...
        if($data_array['db_type'] === 'oci8') {
            ...
            $cnt = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=".$data_array['db_host'].")(PORT=".$data_array['db_port']."))(CONNECT_DATA=(SID=".$data_array['db_name'].")))";
        }
        if($data_array['db_persistency'] === 'yes') {
            ...
            if($data_array['db_type'] === 'oci8' and $data_array['db_port'] != 0) {
                $db2 = $adodb->PConnect($cnt,$data_array['db_user_name'],$data_array['db_password']);
            } elseif($data_array['db_type'] === 'pdo') {
                $db2 = $adodb->PConnect('mysql:host='.$data_array['db_host'].';dbname='.$data_array['db_name'],$data_array['db_user_name'],$data_array['db_password']);
            } else {
            ...
            if($data_array['db_type'] === 'oci8' and $data_array['db_port'] != 0) {
                ...
                $db2 = $adodb->Connect('mysql:host='.$data_array['db_host'].';dbname='.$data_array['db_name'],$data_array['db_user_name'],$data_array['db_password']);
            } else {
        ...
        for($i = 0,$size = count($install_lang);$i < $size;++$i) {
            if($data_array['db_type'] === 'mysqli' or $data_array['db_type'] === 'pdo' or $data_array['db_type'] === 'mysql' or $data_array['db_type'] === 'mysqlt' or $data_array['db_type'] === 'maxsql') {
                if($i == 0) {
                    ...
                    $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_users (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_modules_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_modules_".$install_lang[$i]." (module_name,module_custom_name,module_version,module_status,module_value_view) VALUES
                    ('news', 'News', NULL, '1', '0'),
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_blocks_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_blocks_".$install_lang[$i]." (block_name,block_custom_name,block_version,block_status,block_value_view,block_content,block_url,block_arrangements,block_priority,block_refresh,block_last_refresh,block_term_expire,block_term_expire_action) VALUES ('menu', 'Navigation', NULL, '1', '0', NULL, NULL, 'left', 1, NULL, NULL, NULL, NULL);");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                //shop start
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_catalogs_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_categories_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_subcategories_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_items_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_delivery_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_delivery_".$install_lang[$i]." (delivery_name,delivery_price,delivery_status) VALUES ('personal pickup', '0.00', '1');");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                }
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_pay_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_pay_".$install_lang[$i]." (pay_name,pay_price,pay_status) VALUES ('cash', '0.00', '1');");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                }
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_orders_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_order_items_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_mail_receivers_".$install_lang[$i]." (
                ...
                if($i == 0) {
                    $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_mail_sendlog (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_main_menu_".$install_lang[$i]." (
                ...
                if($i == 0) {
                    $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_session (
                    ...
                    $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_system_pages (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_messages_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_rss_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_news_".$install_lang[$i]." (
                ...
                $insert = $adodb->Execute("CREATE TABLE ".$data_array['prefix']."_news_categories_".$install_lang[$i]." (
                ...
                if($i == 0) {
                    ...
                    $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_users ADD (
                        PRIMARY KEY(user_id),
                    ...
                    $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_users_id
                        START WITH 1
                        INCREMENT BY 1
                        MINVALUE 1
                        NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_modules_".$install_lang[$i]." ADD (
                    PRIMARY KEY(module_id),
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_modules_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_modules_".$install_lang[$i]." VALUES (".$adodb->GenID($data_array['prefix']."_modules_id_".$install_lang[$i]).", 'news', 'News', NULL, '1', '0')");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                }
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_modules_".$install_lang[$i]." VALUES (".$adodb->GenID($data_array['prefix']."_modules_id_".$install_lang[$i]).", 'main_pages', 'Main Pages', NULL, '1', '0')");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_blocks_".$install_lang[$i]." ADD (
                    PRIMARY KEY(block_id),
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_blocks_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_blocks_".$install_lang[$i]." VALUES (".$adodb->GenID($data_array['prefix']."_blocks_id_".$install_lang[$i]).", 'menu', 'Navigation', NULL, '1', '0', NULL, NULL, 'left', 1, NULL, NULL, NULL, NULL)");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_catalogs_".$install_lang[$i]." ADD (
                    PRIMARY KEY(catalog_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_catalogs_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_categories_".$install_lang[$i]." ADD (
                    PRIMARY KEY(category_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_categories_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_subcategories_".$install_lang[$i]." ADD (
                    PRIMARY KEY(subcategory_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_subcategories_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_items_".$install_lang[$i]." ADD (
                    PRIMARY KEY(item_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_items_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_delivery_".$install_lang[$i]." ADD (
                    PRIMARY KEY(delivery_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_delivery_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_delivery_".$install_lang[$i]." VALUES (".$adodb->GenID($data_array['prefix']."_delivery_id_".$install_lang[$i]).", 'personal pickup', '0.00', '1')");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_pay_".$install_lang[$i]." ADD (
                    PRIMARY KEY(pay_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_pay_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_pay_".$install_lang[$i]." VALUES (".$adodb->GenID($data_array['prefix']."_pay_id_".$install_lang[$i]).", 'cash', '0.00', '1')");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                    $error_code = $adodb->ErrorNo();
                    $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_orders_".$install_lang[$i]." ADD (
                    PRIMARY KEY(order_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_orders_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_order_items_".$install_lang[$i]." ADD (
                    PRIMARY KEY(order_item_id),
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_order_items_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                if($i == 0) {
                    ...
                    $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_guestbook ADD (
                        PRIMARY KEY(post_id)
                      )");
                    if(!$insert) {
                        $error_message = $adodb->ErrorMsg();
                    ...
                    $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_guestbook_id
                        START WITH 1
                        INCREMENT BY 1
                        MINVALUE 1
                        NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_mail_receivers_".$install_lang[$i]." ADD (
                    PRIMARY KEY(receiver_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_mail_receivers_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                if($i == 0) {
                    ...
                    $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_mail_sendlog ADD (
                        PRIMARY KEY(mail_id)
                      )");
                    if(!$insert) {
                        $error_message = $adodb->ErrorMsg();
                    ...
                    $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_mail_sendlog_id
                        START WITH 1
                        INCREMENT BY 1
                        MINVALUE 1
                        NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_main_menu_".$install_lang[$i]." ADD (
                    PRIMARY KEY(main_menu_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_main_menu_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                if($i == 0) {
                    ...
                    $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_system_pages ADD (
                        PRIMARY KEY(system_page_id)
                      )");
                    if(!$insert) {
                        $error_message = $adodb->ErrorMsg();
                    ...
                    $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_system_pages_id
                        START WITH 1
                        INCREMENT BY 1
                        MINVALUE 1
                        NOCACHE
                    ...
                    $systempage_id = $adodb->GenID($data_array['prefix']."_system_pages_id");
                    $insert = $adodb->Execute("INSERT INTO ".$data_array['prefix']."_system_pages VALUES (".$systempage_id.", 'admin', 'account_rules', 'General rules', '".time()."', empty_clob(), 'en',null,null,null)");
                    if(!$insert) {
                        $error_message = $adodb->ErrorMsg();
                        $error_code = $adodb->ErrorNo();
                        $error[] = array("code" => $error_code,"message" => $error_message);
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_messages_".$install_lang[$i]." ADD (
                    PRIMARY KEY(message_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_messages_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_rss_".$install_lang[$i]." ADD (
                    PRIMARY KEY(rss_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_rss_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_main_pages_".$install_lang[$i]." ADD (
                    PRIMARY KEY(main_page_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_main_pages_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_news_".$install_lang[$i]." ADD (
                    PRIMARY KEY(news_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_news_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
                ...
                $insert = $adodb->Execute("ALTER TABLE ".$data_array['prefix']."_news_categories_".$install_lang[$i]." ADD (
                    PRIMARY KEY(news_category_id)
                  )");
                if(!$insert) {
                    $error_message = $adodb->ErrorMsg();
                ...
                $insert = $adodb->Execute("CREATE SEQUENCE ".$data_array['prefix']."_news_categories_id_".$install_lang[$i]."
                    START WITH 1
                    INCREMENT BY 1
                    MINVALUE 1
                    NOCACHE
    ...
    public function step6() {
        ...
        if((!isset($data_array['admin_add_email']) or !isset($data_array['admin_add_login']) or !isset($data_array['admin_add_password']) or !isset($data_array['admin_add_password2']) or !isset($data_array['admin_add_viewemail'])) or ($data_array['admin_add_email'] == "" or $data_array['admin_add_login'] == "" or $data_array['admin_add_password'] == "" or $data_array['admin_add_password2'] == "" or $data_array['admin_add_viewemail'] == "")) {
            $this->smarty->assign("error","not_all_data");
            $this->smarty->display("install/action.html","|not_all_data");
            exit();
        }
        ...
        $sql = "INSERT INTO ".PREFIX."_users (".$field_user_id."user_temp,user_email,user_login,user_password,user_rights,user_gmt,user_active,user_reg_date,user_view_email)  VALUES (".$admin_add_id."'0',".$data_array['admin_add_email'].",".$data_array['admin_add_login'].",'".$data_array['admin_add_password']."','main_admin','+00','1','".$now."',".$data_array['admin_add_viewemail'].")";
        $result = $adodb->Execute($sql);
        if($result === false or $adodb->Affected_Rows() == 0) {
            $error_message = $adodb->ErrorMsg();
            $error_code = $adodb->ErrorNo();
            $error[] = array("code" => $error_code,"message" => $error_message);
            if(DB_PERSISTENCY === 'no') {
        ...
        if(is_dir('../themes/systemdk/languages')) {
            if($file_read = opendir('../themes/systemdk/languages')) {
                ...
                while(false !== ($file = readdir($file_read))) {
                    if(!preg_match("/[.]/",$file)) {
                        $lang_list[] = array("item" => $i,"name" => $file);
                        $i++;
                    }
                }
                closedir($file_read);
