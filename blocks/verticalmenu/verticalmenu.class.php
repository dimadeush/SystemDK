 ...
 * @version   3.0
 */
class block_verticalmenu_model extends model_base {
    ...
    private function getConfigVars($index) {
        return $this->registry->main_class->getConfigVars($index);
    }


    public function index() {
        $this->assign("systemdk_block_vertical_menu","yes");
        ...
        if(isset($numrows) and $numrows > 0) {
            ...
            while(!$result->EOF) {
                ...
                if($mainmenu_element_type === "parent") {
                    $mainmenu_id = intval($result->fields['7']);
                    ...
                    if(!isset($mainmenu_id)) {
                        ...
                        $mainmenu_parent_name = $this->getConfigVars('_OTHERPAGELINK');
