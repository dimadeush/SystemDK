...
class guestbook extends model_base {
    ...
    public function index($num_page = false) {
        ...
        if(isset($num_page) and intval($num_page) != 0) {
        ...
        if($result) {
            ...
            $sql = "SELECT a.post_id,a.post_user_name,a.post_user_country,a.post_user_city,a.post_user_telephone,a.post_show_telephone,a.post_user_email,a.post_show_email,a.post_user_ip,a.post_date,a.post_text,a.post_answer,a.post_answer_date,a.post_user_id FROM ".PREFIX."_guestbook a WHERE a.post_status = '1' ORDER BY a.post_date ".$systemdk_guestbook_sortorder;
    ...
    public function add() {
        ...
        if($this->registry->main_class->is_user()) {
            ...
            if(!empty($row) and intval($row['0']) > 0) {
                ...
                if(empty($guestbook_user_name) and !empty($guestbook_user_surname)) {
                    $guestbook_user_name = $guestbook_user_surname;
                } elseif(empty($guestbook_user_name) and empty($guestbook_user_surname)) {
    ...
    public function insert($post_array) {
        ...
        $this->result['systemdk_guestbook_metadescription'] = $this->systemdk_guestbook_metadescription;
        $this->result['systemdk_guestbook_metakeywords'] = $this->systemdk_guestbook_metakeywords;
        if((!isset($post_array['post_user_name']) or !isset($post_array['post_user_country']) or !isset($post_array['post_user_city']) or !isset($post_array['post_user_telephone']) or !isset($post_array['post_user_email']) or !isset($post_array['post_text'])) or ($data_array['post_user_name'] == "" or $data_array['post_user_email'] == "" or $data_array['post_text'] == "")) {
        ...
        if(isset($this->systemdk_guestbook_antispam_ipaddr) and $this->systemdk_guestbook_antispam_ipaddr == "yes") {
            ...
            if($result) {
                ...
                if($num_rows >= intval($this->systemdk_guestbook_antispam_num) and intval($this->systemdk_guestbook_antispam_num) > 0) {
        ...
        if(!empty($guestbook_user_name) and $guestbook_user_name !== "no") {
            $data_array['post_user_name'] = $guestbook_user_name;
        }
        if(!empty($guestbook_user_email) and $guestbook_user_email !== "no") {
        ...
        if($check_db_need_lobs == 'yes') {
            ...
            $this->db->StartTrans();
            $sql = "INSERT INTO ".PREFIX."_guestbook(post_id,post_user_name,post_user_country,post_user_city,post_user_telephone,post_show_telephone,post_user_email,post_show_email,post_user_id,post_user_ip,post_date,post_text,post_status,post_answer,post_answer_date)  VALUES ('".$post_add_id."',".$data_array['post_user_name'].",".$data_array['post_user_country'].",".$data_array['post_user_city'].",".$data_array['post_user_telephone'].",'".$data_array['post_show_telephone']."',".$data_array['post_user_email'].",'".$data_array['post_show_email']."',".$guestbook_user_id.",'".$this->registry->ip_addr."','".$now."',empty_clob(),'".$post_status."',NULL,NULL)";
            ...
            $sql = "INSERT INTO ".PREFIX."_guestbook(post_user_name,post_user_country,post_user_city,post_user_telephone,post_show_telephone,post_user_email,post_show_email,post_user_id,post_user_ip,post_date,post_text,post_status,post_answer,post_answer_date)  VALUES (".$data_array['post_user_name'].",".$data_array['post_user_country'].",".$data_array['post_user_city'].",".$data_array['post_user_telephone'].",'".$data_array['post_show_telephone']."',".$data_array['post_user_email'].",'".$data_array['post_show_email']."',".$guestbook_user_id.",'".$this->registry->ip_addr."','".$now."','".$data_array['post_text']."','".$post_status."',NULL,NULL)";
            $insert_result = $this->db->Execute($sql);
            if($insert_result === false) {
                $error_message = $this->db->ErrorMsg();
                $error_code = $this->db->ErrorNo();
