...
class controller_account extends controller_base
{
    ...
    public function process_autoload()
    {
        ...
        if ($error === 'login' || $error === 'check_code' || $error === 'user_not_active' || $error === 'user_incorrect_pass' || $error === 'user_not_found'
            || $error
               === 'user_not_all_data'
        ) {
    ...
    {
        if ($this->registry->main_class->is_user() && $this->registry->router->get_action() !== 'index') {
            if (in_array(
                $this->registry->router->get_action(), [
                    "account_register",
                    "account_add_save",
                    "account_add_confirm",
                    "account_lostpass",
    ...
    private function account_view($type, $title, $back_link = false, $cache_category = false, $cache = false, $template = false)
    {
        ...
        if ($template === 'accountenter.html' || $template === 'account_add.html' || $template === 'account_addconf.html' || $template === 'account_lostpass.html'
    ...
    public function account_register()
    {
        ...
        if (!empty($error) && ($error === 'register_not_allow' || $error === 'max_users_limit')) {
            $this->account_view($error, $title);
        }
        $register_rules_accept = false;
        if (isset($_GET['rules']) && intval($_GET['rules']) == 1) {
            $register_rules_accept = "yes";
        }
        if (isset($_POST['register_rules_accept']) && $this->registry->main_class->format_htmlspecchars_stripslashes_striptags($_POST['register_rules_accept']) != "") {
        ...
        if (defined('SYSTEMDK_ACCOUNT_RULES') && SYSTEMDK_ACCOUNT_RULES == "yes" && !$register_rules_accept) {
            ...
            if (!$this->isCached("index.html", $this->registry->sitelang . "|" . $cache_category . "|" . $cache . "|" . $this->registry->language)) {
                ...
                $error = $this->model_account->get_property_value('error');
                $result = $this->model_account->get_property_value('result');
                if (!empty($error) && $error === 'account_rules_not_found') {
                    $this->account_view($error, $title);
                }
                $this->assign_array($result);
            }
        ...
        if (defined('SYSTEMDK_ACCOUNT_RULES') && SYSTEMDK_ACCOUNT_RULES == "yes" && $register_rules_accept == 'no') {
            $this->account_view('rules_not_accept', $title);
        } elseif ((defined('SYSTEMDK_ACCOUNT_RULES') && SYSTEMDK_ACCOUNT_RULES == "yes" && $register_rules_accept == 'yes')
                  || (defined('SYSTEMDK_ACCOUNT_RULES')
                      && SYSTEMDK_ACCOUNT_RULES == "no")
        ) {
            $type = false;
            $back_link = false;
            $cache_category = 'modules|account|register';
    ...
    public function account_add_save()
    {
        ...
        $result = $this->model_account->get_property_value('result');
        if (isset($result['need_send_email']) && $result['need_send_email'] === 'yes') {
            if (isset($result['insert_result']) && $result['insert_result']) {
                $this->configLoad($this->registry->language . "/modules/account/account.conf");
                $this->assign_array($result);
        ...
        if (!empty($error) && ($error === 'reg_not_all_data' || $error === 'register_not_allow' || $error === 'max_users_limit' || $error === 'sql_error')) {
            if (!empty($error_array) && is_array($error_array)) {
                $this->assign("errortext", $error_array);
            }
            $this->account_view($error, $title);
        } elseif (!empty($error)
                  && ($error === 'reg_check_code' || $error === 'reg_save_user_email' || $error === 'reg_login' || $error === 'reg_pass'
                      || $error
                         === 'reg_found_user')
        ) {
    ...
    public function account_add_confirm()
    {
        ...
        $error_array = $this->model_account->get_property_value('error_array');
        $result = $this->model_account->get_property_value('result');
        if ($error === 'conf_reg_check_code' || $error === 'no_act_code' || $error === 'add_conf_sql_error') {
    ...
    public function account_lostpassconf()
    {
        ...
        $error_array = $this->model_account->get_property_value('error_array');
        $result = $this->model_account->get_property_value('result');
        if (!empty($result['need_send_email']) && $result['need_send_email'] === 'yes') {
        ...
        if ($error === 'pass_not_all_data' || $error === 'pass_check_code' || $error === 'pass_not_found' || $error === 'lost_pass_sql_error'
            || $error
               === 'lost_pass_not_update'
        ) {
            if (!empty($error_array) && is_array($error_array)) {
                $this->assign("errortext", $error_array);
            }
            ...
            } elseif ($error === 'pass_not_found' || $error === 'lost_pass_not_update') {
                $back_link = 'lost_pass';
            }
            $this->account_view($error, $title, $back_link);
        }
    ...
    public function account_newpassconf()
    {
        ...
        $error_array = $this->model_account->get_property_value('error_array');
        $result = $this->model_account->get_property_value('result');
        if ($error === 'new_pass_conf_check_code' || $error === 'new_pass_conf_no_act_code' || $error === 'lost_pass_sql_error'
            || $error
               === 'new_pass_conf_not_update'
        ) {
            if (!empty($error_array) && is_array($error_array)) {
                $this->assign("errortext", $error_array);
            }
            if ($error === 'new_pass_conf_check_code' || $error === 'new_pass_conf_not_update') {
                $back_link = 'conf_user_reg';
            }
            $this->account_view($error, $title, $back_link);
        } elseif (!empty($result['account_message']) && $result['account_message'] === 'new_pass_conf_ok') {
    ...
    public function account_edit_save()
    {
        ...
        $error_array = $this->model_account->get_property_value('error_array');
        $result = $this->model_account->get_property_value('result');
        if ($error === 'edit_not_all_data' || $error === 'edit_email' || $error === 'edit_pass' || $error === 'find_such_user' || $error === 'edit_sql_error'
            || $error
               === 'edit_not_updated'
        ) {
            if (!empty($error_array) && is_array($error_array)) {
