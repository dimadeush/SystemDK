...
class feedback extends model_base
{
    ...
    public function index($get_receivers = true)
    {
        ...
        if (defined('SYSTEMDK_FEEDBACK_RECEIVER') && SYSTEMDK_FEEDBACK_RECEIVER != 'default' && $get_receivers) {
            ...
            if ($result) {
                ...
                if ($row_exist > 0) {
                    while (!$result->EOF) {
                        ...
                        $receivers_all[] = ["receiver_id" => $receiver_id, "receiver_name" => $receiver_name];
                        $result->MoveNext();
                    }
                    $this->result['receivers_all'] = $receivers_all;
                }
            }
        } elseif (!defined('SYSTEMDK_FEEDBACK_RECEIVER') || SYSTEMDK_FEEDBACK_RECEIVER == 'default') {
            $this->result['receivers_all'] = 'default';
        }
        ...
        if ($this->registry->main_class->is_user()) {
            ...
            if (!empty($row) && intval($row['0']) > 0) {
                ...
                if ((!isset($feedback_user_name) || $feedback_user_name == "") && (isset($feedback_user_surname) && $feedback_user_surname != "")) {
                    $feedback_user_name = $feedback_user_surname;
                } elseif ((!isset($feedback_user_name) || $feedback_user_name == "") && (!isset($feedback_user_surname) || $feedback_user_surname == "")) {
    ...
    public function feedback_send($post_array, $process_check = true, $feedback_content = false)
    {
        ...
        if ($process_check) {
            ...
            if ((!isset($post_array['feedback_user_name']) || !isset($post_array['feedback_user_id']) || !isset($post_array['feedback_user_email'])
                 || (!isset($post_array['feedback_towhom']) && SYSTEMDK_FEEDBACK_RECEIVER == 'list')
                 || !isset($post_array['feedback_subject'])
                 || !isset($post_array['feedback_message']))
                || ($data_array['feedback_user_name'] == "" || $data_array['feedback_user_email'] == ""
                    || (isset($data_array['feedback_towhom']) && $data_array['feedback_towhom'] == 0 && SYSTEMDK_FEEDBACK_RECEIVER == 'list')
                    || $data_array['feedback_subject'] == ""
                    || $data_array['feedback_message'] == "")
            ) {
            ...
            if (ENTER_CHECK == "yes") {
                if (extension_loaded("gd")
                    && (!isset($data_array["captcha"])
                        || (isset($_SESSION["captcha"]) && $_SESSION["captcha"] !== $this->registry->main_class->format_striptags(trim($data_array["captcha"])))
            ...
            if (defined('SYSTEMDK_IPANTISPAM') && SYSTEMDK_IPANTISPAM == "yes") {
                $mail_date_check = $this->registry->main_class->get_time() - 86400;
                $sql = "SELECT count(*) FROM " . PREFIX . "_mail_sendlog WHERE mail_ip = '" . $this->registry->ip_addr
                       . "' and mail_module = 'feedback' and mail_date > " . $mail_date_check;
                $result = $this->db->Execute($sql);
                if ($result) {
                    if (isset($result->fields['0'])) {
                    ...
                    if ($num_rows >= intval(SYSTEMDK_IPANTISPAM_NUM) && intval(SYSTEMDK_IPANTISPAM_NUM) > 0) {
            ...
            if ($this->registry->main_class->is_user()) {
                ...
                if (!empty($row) && intval($row['0']) > 0) {
                    ...
                    if (empty($data_array['feedback_user_name']) && !empty($data_array['feedback_user_surname'])) {
                        $data_array['feedback_user_name'] = $data_array['feedback_user_surname'];
                    } elseif (empty($data_array['feedback_user_name']) && empty($data_array['feedback_user_surname'])) {
                ...
                $sql = "SELECT count(*) as count FROM " . PREFIX . "_users WHERE lower(user_email) = " . strtolower($data_array['feedback_user_email'])
                       . " or lower(user_login) = " . strtolower($data_array['feedback_user_name']);
                $result = $this->db->Execute($sql);
                if ($result) {
                    if (isset($result->fields['0'])) {
                        $num_rows = intval($result->fields['0']);
            ...
            if (defined('SYSTEMDK_FEEDBACK_RECEIVER') && SYSTEMDK_FEEDBACK_RECEIVER != 'default') {
                $sql = "SELECT receiver_id,receiver_name,receiver_email FROM " . PREFIX . "_mail_receivers_" . $this->registry->sitelang . " WHERE receiver_id = '"
                       . $data_array['feedback_towhom'] . "' and receiver_active = '1'";
                $result = $this->db->Execute($sql);
                if ($result) {
                    if (isset($result->fields['0'])) {
                        $row_exist = intval($result->fields['0']);
            ...
            if ($this->registry->mail->send_error) {
                ...
                $sql = "INSERT INTO " . PREFIX . "_mail_sendlog (" . $sequence_array['field_name_string'] . "mail_user,mail_date,mail_module,mail_ip) VALUES ("
                       . $sequence_array['sequence_value_string'] . "" . $this->send_mail_data['feedback_user_login'] . "," . $now . ",'feedback','"
                       . $this->registry->ip_addr . "')";
                $result = $this->db->Execute($sql);
                if ($result === false) {
