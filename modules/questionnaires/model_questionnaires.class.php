...
class questionnaires extends model_base
{
    ...
    public function index()
    {
        ...
        $sql = "SELECT questionnaire_id,questionnaire_name,questionnaire_date FROM " . PREFIX . "_q_questionnaires_" . $this->registry->sitelang
               . " WHERE questionnaire_status = '1' ORDER BY questionnaire_date ASC";
    ...
    {
        $sql
            = "SELECT a.questionnaire_id,a.questionnaire_name,a.questionnaire_date,a.questionnaire_meta_title,a.questionnaire_meta_keywords,a.questionnaire_meta_description,b.group_id,b.group_name,c.question_id,c.question_title,c.type_id,c.question_priority,c.question_subtext,c.question_obligatory,d.question_answer_id,d.question_answer,d.question_answer_priority,e.question_item_id,e.question_item_title,e.question_item_priority
                FROM " . PREFIX . "_q_questionnaires_" . $this->registry->sitelang . " a LEFT OUTER JOIN " . PREFIX . "_q_groups_" . $this->registry->sitelang
              . " b on (a.questionnaire_id=b.questionnaire_id and b.group_status = '1') LEFT OUTER JOIN " . PREFIX . "_q_questions_" . $this->registry->sitelang
              . " c on a.questionnaire_id=c.questionnaire_id LEFT OUTER JOIN " . PREFIX . "_q_question_answers_" . $this->registry->sitelang
              . " d on c.question_id=d.question_id LEFT OUTER JOIN " . PREFIX . "_q_question_items_" . $this->registry->sitelang . " e on c.question_id=e.question_id
                WHERE a.questionnaire_id = " . $questionnaire_id
              . " and a.questionnaire_status = '1' ORDER BY c.question_priority,d.question_answer_priority,e.question_item_priority ASC";
        $result = $this->db->Execute($sql);
        if (!$result) {
            $this->error = 'questionnaires_error';
        ...
        while (!$result->EOF) {
            ...
            if ($question_answer_id < 1) {
                ...
                $question_answer = $this->registry->main_class->format_htmlspecchars(
                    $this->registry->main_class->extracting_data($result->fields['15'])
                );
            ...
            if ($question_item_id < 1) {
                ...
                $question_item_title = $this->registry->main_class->format_htmlspecchars(
                    $this->registry->main_class->extracting_data($result->fields['18'])
                );
                $question_item_priority = intval($result->fields['19']);
                $results['questions'][$question_id]['question_items'][$question_item_id] = [
    ...
    public function vote($array)
    {
        ...
        if (empty($array['questionnaire_id']) || empty($array['group_id'])) {
        ...
        foreach ($results['questions'] as $key => $value) {
            ...
            $answer_text = false;
            if (in_array($type_id, [1, 2, 3, 4, 5, 6])) {
                if (in_array($type_id, [1, 3, 4, 6]) && is_array($question_items)) {
                    ...
                    foreach ($question_items as $key2 => $value2) {
                        ...
                        if (in_array($type_id, [3, 6])) {
                            if (isset($_POST[$post_question_answer][$key2]) && array_key_exists(intval($_POST[$post_question_answer][$key2]), $question_answers)) {
                                $answer_id = intval($_POST[$post_question_answer][$key2]);
                            }
                        }
                    }
                    if (empty($question_item_id) && $question_obligatory == 1) {
                        $this->error = 'not_all_data';

                        return;
                    }
                    if (in_array($type_id, [3, 6]) && empty($answer_id) && $question_obligatory == 1) {
                        $this->error = 'not_all_data';

                        return;
                    }
                } elseif (in_array($type_id, [2, 5])) {
                    $question_item_id = false;
                ...
                if (empty($answer_text) && $question_obligatory == 1) {
                    $this->error = 'not_all_data';
        ...
        if ($this->registry->main_class->is_user()) {
            ...
            if (!empty($current_user_id) && intval($current_user_id) > 0) {
                $user_id = "'" . intval($current_user_id) . "'";
            }
        }
        $this->db->StartTrans();
        $sql = "SELECT count(*) FROM " . PREFIX . "_q_voted_groups_" . $this->registry->sitelang . " WHERE group_id = '" . $array['group_id']
               . "' and questionnaire_id = '" . $array['questionnaire_id'] . "'";
        $result = $this->db->Execute($sql);
        if (!$result) {
            $this->error_array[] = ["code" => $this->db->ErrorNo(), "message" => $this->db->ErrorMsg()];
        ...
        if ($row_exist < 1) {
            ...
            $sql = "INSERT INTO " . PREFIX . "_q_voted_groups_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
                   . "group_id,questionnaire_id) VALUES (" . $sequence_array['sequence_value_string'] . "'" . $array['group_id'] . "','" . $array['questionnaire_id']
                   . "')";
        ...
        $sql = "INSERT INTO " . PREFIX . "_q_voted_persons_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
               . "user_id,voted_group_id,person_ip,voted_date) VALUES (" . $sequence_array['sequence_value_string'] . "" . $user_id . ",(SELECT voted_group_id FROM "
               . PREFIX . "_q_voted_groups_" . $this->registry->sitelang . " WHERE group_id = '" . $array['group_id'] . "' and questionnaire_id = '"
               . $array['questionnaire_id'] . "'),'" . $this->registry->ip_addr . "','" . $this->registry->main_class->get_time() . "')";
        $insert_result = $this->db->Execute($sql);
        if ($insert_result === false) {
            $this->error_array[] = ["code" => $this->db->ErrorNo(), "message" => $this->db->ErrorMsg()];
            $this->error = 'sql_error';
        ...
        foreach ($results['questions'] as $key => $value) {
                ...
                if (in_array($type_id, [1, 3, 4, 6]) && is_array($question_items)) {
                    foreach ($question_items as $key2 => $value2) {
                        ...
                        if (in_array($type_id, [3, 6])) {
                            ...
                            if (!empty($_POST[$post_question_answer][$key2]) && array_key_exists(intval($_POST[$post_question_answer][$key2]), $question_answers)) {
                                $question_item_answer_id = "'" . intval($_POST[$post_question_answer][$key2]) . "'";
                            }
                        } else {
                            $question_item_answer_id = 'NULL';
                        }
                        if (!empty($question_item_answer_id) && !empty($question_item_id)) {
                            $result = $this->process_question_item(
                                [
                                    'person_id'               => $person_id,
                                    'question_id'             => $question_id,
                                    'question_item_id'        => $question_item_id,
            ...
            if (in_array(
                ...
                && !empty($_POST[$post_question_text])
                && $this->registry->main_class->format_striptags(trim($_POST[$post_question_text])) != ''
            ) {
                $result = $this->process_answer_text(
                    [
        ...
        if ($this->systemdk_questionnaires_voted_method_check == 'cookie') {
            ...
            $systemdk_questionnaires_voted_control_live_time = intval($this->systemdk_questionnaires_voted_control_live_time) > 0 ? intval(
                $this->systemdk_questionnaires_voted_control_live_time
            ) : 31536000; // if revote disable, we just set cookie live time to one year
            setcookie(
                "voted_cookie_" . $array['questionnaire_id'] . "_" . $array['group_id'] . "_" . $this->registry->sitelang, "$voted_cookie",
                time() + $systemdk_questionnaires_voted_control_live_time, "/" . SITE_DIR
            );
        }
        ...
        $this->registry->main_class->clearCache(
            null,
            $this->registry->sitelang . "|systemadmin|modules|questionnaires|results_details|questionnaire|" . $array['questionnaire_id'] . "|group|" . $array['group_id']
        );
    ...
    private function process_question_item($data_array)
    {
        ...
        $sql = "INSERT INTO " . PREFIX . "_q_voted_answers_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
               . "person_id,question_id,question_item_id,questionnaire_id,answer_text,group_id,question_answer_id) VALUES (" . $sequence_array['sequence_value_string']
               . "'"
               . $data_array['person_id'] . "','" . $data_array['question_id'] . "','" . $data_array['question_item_id'] . "','" . $data_array['questionnaire_id'] . "',"
    ...
    {
        $answer_text = $this->registry->main_class->processing_data(
            $this->registry->main_class->format_striptags(trim($_POST[$data_array['post_question_text']]))
        );
        ...
        $sql = "INSERT INTO " . PREFIX . "_q_voted_answers_" . $this->registry->sitelang . " (" . $sequence_array['field_name_string']
               . "person_id,question_id,question_item_id,questionnaire_id,answer_text,group_id,question_answer_id) VALUES (" . $sequence_array['sequence_value_string']
               . "'"
               . $data_array['person_id'] . "','" . $data_array['question_id'] . "',NULL,'" . $data_array['questionnaire_id'] . "'," . $answer_text_str . ",'"
               . $data_array['group_id'] . "',NULL)";
        $insert_result = $this->db->Execute($sql);
        if ($insert_result === false) {
            $this->error_array[] = ["code" => $this->db->ErrorNo(), "message" => $this->db->ErrorMsg()];
            $this->error = 'sql_error';
        ...
        if ($check_db_need_lobs == 'yes') {
            $insert_result2 = $this->db->UpdateClob(
                PREFIX . '_q_voted_answers_' . $this->registry->sitelang, 'answer_text', $answer_text, 'answer_id=' . $sequence_array['sequence_value']
            );
    ...
    {
        if (in_array($this->systemdk_questionnaires_voted_method_check, ['ip_address', 'cookie'])) {
            ...
            if ($this->systemdk_questionnaires_voted_method_check == 'ip_address') {
                ...
                $sql = "SELECT count(*) FROM " . PREFIX . "_q_voted_persons_" . $this->registry->sitelang . " a," . PREFIX . "_q_voted_groups_"
                       . $this->registry->sitelang . " b WHERE a.voted_group_id = b.voted_group_id and a.person_ip = '" . $this->registry->ip_addr
                       . "' and b.group_id = '"
                       . $group_id . "' and b.questionnaire_id = '" . $questionnaire_id . "' " . $sql_add;
                $result = $this->db->Execute($sql);
                if (!$result) {
                    $this->error = 'questionnaires_error';
                ...
                if (!empty($_COOKIE['voted_cookie_' . $questionnaire_id . '_' . $group_id . '_' . $this->registry->sitelang])) {
                    $voted_cookie = explode(
                        ":", base64_decode($_COOKIE['voted_cookie_' . $questionnaire_id . '_' . $group_id . '_' . $this->registry->sitelang])
                    );
                    $voted_questionnaire_id = intval($voted_cookie[0]);
                    $voted_group_id = intval($voted_cookie[1]);
                }
            }
            if (!empty($voted_questionnaire_id) && !empty($voted_group_id) && $questionnaire_id == $voted_questionnaire_id && $group_id == $voted_group_id) {
                $this->error = 'already_voted';

                return false;
            }
        }
        if ($this->systemdk_questionnaires_who_can_vote == 'only_users' && !$this->registry->main_class->is_user()) {
            $this->error = 'not_user';
