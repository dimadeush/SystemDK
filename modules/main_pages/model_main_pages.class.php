...
class main_pages extends model_base {
    ...
    public function process_autoload() {
        ...
        $clear = 'no';
        $this->db->Execute("UPDATE ".PREFIX."_main_pages_".$this->registry->sitelang." SET main_page_status = '0', main_page_term_expire = NULL, main_page_term_expire_action = NULL WHERE main_page_term_expire is not NULL and main_page_term_expire <= ".$now." and main_page_term_expire_action = 'off'");
        ...
        $sql = "SELECT main_page_id FROM ".PREFIX."_main_pages_".$this->registry->sitelang." WHERE main_page_term_expire is not NULL and main_page_term_expire <= ".$now." and main_page_term_expire_action = 'delete'";
        ...
        if($result) {
            ...
            if($row_exist > 0) {
                while(!$result->EOF) {
                    ...
                    $sql2 = "SELECT main_menu_priority FROM ".PREFIX."_main_menu_".$this->registry->sitelang." WHERE (main_menu_module = 'main_pages' or main_menu_submodule = 'main_pages') and main_menu_add_param = '".$main_page_id."'";
                    ...
                    if($result2) {
                        ...
                        if($mainmenu_priority > 0) {
                            $this->db->Execute("UPDATE ".PREFIX."_main_menu_".$this->registry->sitelang." SET main_menu_priority=main_menu_priority-1 WHERE main_menu_priority > ".$mainmenu_priority);
                            $num_result3 = $this->db->Affected_Rows();
                            $sql4 = "DELETE FROM ".PREFIX."_main_menu_".$this->registry->sitelang." WHERE (main_menu_module = 'main_pages' or main_menu_submodule = 'main_pages') and main_menu_add_param = '".$main_page_id."'";
    ...
    public function index($main_page_id,$num_page = false) {
        ...
        $sql = "SELECT main_page_id,main_page_author,main_page_title,main_page_date,main_page_content,main_page_notes,main_page_value_view,main_page_status,main_page_term_expire,main_page_term_expire_action,(SELECT max(main_menu_name) FROM ".PREFIX."_main_menu_".$this->registry->sitelang." WHERE (main_menu_module = 'main_pages' or main_menu_submodule = 'main_pages') and main_menu_add_param = '".$main_page_id."') as main_menu_name FROM ".PREFIX."_main_pages_".$this->registry->sitelang." WHERE main_page_id = '".$main_page_id."'";
        $result = $this->db->Execute($sql);
        if(!$result) {
            $this->error = 'nopage';
        ...
        if(!empty($num_page) and intval($num_page) != 0) {
        ...
        } elseif($mainpage_valueview == "1" and ($this->registry->main_class->is_user() or $this->registry->main_class->is_admin())) {
            $this->result['mainpage_all'] = $all;
        } elseif($mainpage_valueview == "2" and $this->registry->main_class->is_admin()) {
            $this->result['mainpage_all'] = $all;
        } elseif($mainpage_valueview == "3" and (!$this->registry->main_class->is_user() or $this->registry->main_class->is_admin())) {
            $this->result['mainpage_all'] = $all;
        } else {
            $this->error = 'noaccess';
            $this->result['pages_menu'] = 'no';
