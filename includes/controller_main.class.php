...
class controller_main extends controller_base {
    ...
    public function index() {
        ...
        if($result === 'lang_file') {
            ...
            if(defined('__SITE_ADMIN_PART') and __SITE_ADMIN_PART === 'yes') {
                $this->registry->controller_theme->display_theme_systemerror("Fatal Error","SystemDK can't find language files: ".__SITE_PATH."/themes/".SITE_THEME."/languages/".$this->registry->language."/".ADMIN_DIR);
                $this->display("error2.html",$this->registry->sitelang."|fatalerror|admlangfile|".$this->registry->language);
            } else {
                $this->registry->controller_theme->display_theme_systemerror("Fatal Error","SystemDK can't find language files: ".__SITE_PATH."/themes/".SITE_THEME."/languages/".$this->registry->language);
                $this->display("error2.html",$this->registry->sitelang."|fatalerror|langfile|".$this->registry->language);
            }
            exit();
        } elseif($result === 'ban_ip' or $result === 'find_install' or $result === 'version_php') {
            if(defined('__SITE_ADMIN_PART') and __SITE_ADMIN_PART === 'yes') {
    ...
    public function block($block_locate) {
        ...
        if(!empty($blocks_alldata)) {
            foreach($blocks_alldata as &$content) {
                if(!empty($content['block_content']) and ($content['block_content'] === 'error_file' or $content['block_content'] === 'error_content')) {
    ...
    public function block_rss($block_customname,$block_content,$block_url,$block_refresh,$block_lastrefresh,$block_id,$admin_rss_action = false) {
        ...
        if(!empty($result['rss_link']) and $result['rss_link'] === 'no') {
            ...
            $result = $this->model->update_block_rss(array(
                                                          'case' => 'case1',
                                                          'cont' => $cont,
                                                          'block_id' => $block_id,
                                                          'block_url' => $block_url
            ...
            $block_content = "";
            $this->model->update_block_rss(array(
                                                'case' => 'case2',
        ...
        if((isset($result['context']) and ($result['context'] == 1)) or (!empty($block_content))) {
            $this->assign("rss_data","addline");
            $this->assign("rssurl",$rssurl[0]);
            $this->assign("language",$this->registry->language);
            $block_content .= $this->fetch("systemadmin/modules/blocks/blockrss.html");
        }
        if((isset($result['context']) and ($result['context'] == 0)) or (empty($block_content))) {
