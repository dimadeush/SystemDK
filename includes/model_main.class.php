...
class model_main extends model_base {
    ...
    public function index() {
        ...
        if(isset($this->registry->GET_lang) and trim($this->registry->GET_lang) != "" and ((defined('SYSTEMDK_SITE_LANG') and SYSTEMDK_SITE_LANG === "yes") or (defined('SYSTEMDK_ADMIN_SITE_LANG') and SYSTEMDK_ADMIN_SITE_LANG === "yes" and defined('__SITE_ADMIN_PART') and __SITE_ADMIN_PART === 'yes'))) {
            ...
            if(file_exists(__SITE_PATH."/themes/".SITE_THEME."/languages/".$lang) and trim($lang) != "" and file_exists(__SITE_PATH."/includes/data/".$lang."_installed.dat")) {
                $site_lang = $lang;
                if((!isset($this->registry->GET_ilang) or trim(substr($this->format_striptags(trim($this->registry->GET_ilang)),0,2)) == "") and (!isset($_SESSION["ilanguage"]) or trim(substr($this->format_striptags(trim($_SESSION["ilanguage"])),0,2)) == "") and ((defined('SYSTEMDK_INTERFACE_LANG') and SYSTEMDK_INTERFACE_LANG === "yes") or (defined('SYSTEMDK_ADMIN_INTERFACE_LANG') and SYSTEMDK_ADMIN_INTERFACE_LANG === "yes" and defined('__SITE_ADMIN_PART') and __SITE_ADMIN_PART === 'yes'))) {
        ...
        if(isset($this->registry->GET_ilang) and trim($this->registry->GET_ilang) != "" and ((defined('SYSTEMDK_INTERFACE_LANG') and SYSTEMDK_INTERFACE_LANG === "yes") or (defined('SYSTEMDK_ADMIN_INTERFACE_LANG') and SYSTEMDK_ADMIN_INTERFACE_LANG === "yes" and defined('__SITE_ADMIN_PART') and __SITE_ADMIN_PART === 'yes'))) {
            $i_lang = substr($this->format_striptags(trim($this->registry->GET_ilang)),0,2);
            if(file_exists(__SITE_PATH."/themes/".SITE_THEME."/languages/".$i_lang) and trim($i_lang) != "") {
    ...
    public function db_process_sequence($sequence,$for_field) {
        $array = array('sequence_value' => false,'sequence_value_string' => false,'field_name_string' => false);
    ...
    private function process_user() {
        if(isset($this->registry->GET_func) and $this->format_striptags(trim($this->registry->GET_func)) == "account_logout") {
            ...
            if(ENTER_CHECK == "yes" and extension_loaded("gd")) {
                if(empty($this->registry->POST_captcha) or (isset($_SESSION["captcha"]) and isset($this->registry->POST_captcha) and $_SESSION["captcha"] !== $this->format_striptags(trim($this->registry->POST_captcha))) or empty($_SESSION["captcha"])) {
                    $this->clear(array('captcha' => 1,'user' => 1));
                    $this->registry->module_account_error = 'check_code';
                    return false;
                }
                $this->clear(array('captcha' => 1));
            }
            return $this->check_user($user_login,$user_password);
        } elseif((isset($this->registry->POST_user_login) or isset($this->registry->POST_user_password)) and ($this->format_striptags(trim($this->registry->POST_func)) == "mainuser" or $this->format_striptags(trim($this->registry->POST_func == "mainadmin")))) {
            if(ENTER_CHECK == "yes" and extension_loaded("gd")) {
                $this->clear(array('captcha' => 1,'user' => 1));
            } else {
                $this->clear(array('user' => 1));
            ...
            return false;
        } elseif((!empty($_COOKIE['user_cookie']) or !empty($_SESSION['systemdk_user'])) and !isset($this->registry->POST_user_login) and !isset($this->registry->POST_user_password)) {
            $user_id = false;
            $user_session_code = false;
            if(!empty($_SESSION['systemdk_user']) and is_array($_SESSION['systemdk_user'])) {
                $user_session = $_SESSION['systemdk_user'];
                $user_session = array(0 => base64_decode($user_session[0]),1 => base64_decode($user_session[1]));
            ...
            if(empty($user_id) or empty($user_session_code)) {
                $this->clear(array('user' => 1));
    ...
    private function check_user($user_login,$user_password,$user_id = false,$user_session_code = false) {
        if(!empty($user_login) and !empty($user_password)) {
            $try_login = true;
        ...
        $sql = "SELECT user_id, user_name, user_surname, user_login, user_password, user_session_code, user_session_agent, user_rights, user_active, user_activate_key FROM ".PREFIX."_users WHERE user_temp = '0' and ".$where1;
        ...
        if(isset($exist) and $exist > 0) {
            if((($try_login and $this->extracting_data($result->fields['4']) === crypt($user_password,$this->extracting_data($result->fields['4'])) and $this->extracting_data($result->fields['4']) != "") or !$try_login) and intval($result->fields['8']) == 1) {
                $this->user_rights = $this->extracting_data($result->fields['7']);
                if($this->user_rights === 'user') {
                    if(!$try_login or ($try_login and !defined('__SITE_ADMIN_PART'))) {
                        $this->systemdk_is_user_now = 1;
                    } else {
                        $this->registry->module_account_error = 'user_not_found';
                        return false;
                    }
                } elseif($this->user_rights === 'admin' or $this->user_rights === 'main_admin') {
                ...
                if($user_session_code == '' or $try_login) {
                ...
                $_SESSION['systemdk_user'] = array(
                    0 => base64_encode($this->user_id),
                    1 => base64_encode($user_session_code)
                );
                if(empty($this->user_name) and !empty($user_surname)) {
                    $this->user_name = $user_surname;
                } elseif(empty($this->user_name) and empty($user_surname)) {
                ...
                if(!empty($user_activate_key)) {
                    $this->db->Execute("UPDATE ".PREFIX."_users SET user_activate_key = NULL,user_new_password = NULL,user_last_login_date = ".$this->get_time().",user_session_code = '".$user_session_code."',user_session_agent = ".$user_session_agent." WHERE user_id='".$this->user_id."'");
                } else {
                    $this->db->Execute("UPDATE ".PREFIX."_users SET user_last_login_date = ".$this->get_time().",user_session_code = '".$user_session_code."',user_session_agent = ".$user_session_agent." WHERE user_id='".$this->user_id."'");
                }
                return true;
            } elseif((($try_login and $this->extracting_data($result->fields['4']) === crypt($user_password,$this->extracting_data($result->fields['4'])) and $this->extracting_data($result->fields['4']) != "") or !$try_login) and intval($result->fields['8']) == 0) {
                $this->clear(array('user' => 1));
                $this->registry->module_account_error = 'user_not_active';
                return false;
            } elseif($try_login and $this->extracting_data($result->fields['4']) !== crypt($user_password,$this->extracting_data($result->fields['4'])) and $this->extracting_data($result->fields['4']) != "") {
                $this->clear(array('user' => 1));
    ...
    public function path($controller) {
        $sql = "SELECT module_custom_name, module_status, module_value_view, (SELECT max(main_menu_name) FROM ".PREFIX."_main_menu_".$this->registry->sitelang." WHERE (main_menu_module='".$controller."' OR main_menu_submodule='".$controller."') and main_menu_type not in ('main_page')) as main_menu_name,module_id FROM ".PREFIX."_modules_".$this->registry->sitelang." WHERE module_name='".$controller."'";
        $result = $this->db->Execute($sql);
        return $result;
    }
    ...
    public function block($block_locate) {
        ...
        $sql = "SELECT block_id, block_name, block_custom_name, block_value_view, block_content, block_url, block_refresh, block_last_refresh FROM ".PREFIX."_blocks_".$this->registry->sitelang." WHERE block_arrangements = '".$block_locate."' AND block_status = '1' ORDER BY block_priority ASC";
        ...
        if(isset($exist) and $exist > 0) {
            while(!$result_blocks->EOF) {
                ...
                } elseif($block_valueview == "1" and ($this->is_user() or $this->is_admin())) {
                    $blocks_alldata[] = $this->block_render($block_name,$block_customname,$block_content,$block_id,$block_url,$block_refresh,$block_lastrefresh);
                } elseif($block_valueview == "2" and $this->is_admin()) {
                    $blocks_alldata[] = $this->block_render($block_name,$block_customname,$block_content,$block_id,$block_url,$block_refresh,$block_lastrefresh);
                } elseif($block_valueview == "3" and (!$this->is_user() or $this->is_admin())) {
                    $blocks_alldata[] = $this->block_render($block_name,$block_customname,$block_content,$block_id,$block_url,$block_refresh,$block_lastrefresh);
                }
    ...
    private function block_file_include($block_customname,$block_name) {
        ...
        if(!file_exists(__SITE_PATH."/blocks/".$block_name."/block_controller_".$block_name.".class.php") or !file_exists(__SITE_PATH."/blocks/".$block_name."/block_model_".$block_name.".class.php")) {
    ...
    private function who_online() {
        ...
        if($result) {
            $now = $this->get_time();
            $exist = intval($result->fields['0']); //$result->PO_RecordCount(PREFIX."_session","session_login = ".$user_login." and session_hostaddress = '".$ip_addr."'");
            if($exist > 0) {
                $sql = "UPDATE ".PREFIX."_session SET session_login=".$user_login.", session_time='".$now."', session_host_address = '".$ip_addr."', session_who_online='".$who."' WHERE session_login = ".$user_login." and session_host_address = '".$ip_addr."'";
                $this->db->Execute($sql);
            } else {
                $sql = "INSERT INTO ".PREFIX."_session (session_login, session_time, session_host_address, session_who_online) VALUES (".$user_login.", '".$now."', '".$ip_addr."', '".$who."')";
                $this->db->Execute($sql);
            }
        }
    }
    ...
    public function blocks_check_process() {
        $now = $this->get_time();
        $this->db->Execute("UPDATE ".PREFIX."_blocks_".$this->registry->sitelang." SET block_status = '0', block_term_expire = NULL WHERE block_term_expire IS NOT NULL and block_term_expire <= ".$now." and block_term_expire_action = 'off'");
        $num_result1 = $this->db->Affected_Rows();
        $this->db->Execute("DELETE FROM ".PREFIX."_blocks_".$this->registry->sitelang." WHERE block_term_expire IS NOT NULL and block_term_expire <= ".$now." and block_term_expire_action = 'delete'");
    ...
    public function messages_check_process() {
        $now = $this->get_time();
        $this->db->Execute("UPDATE ".PREFIX."_messages_".$this->registry->sitelang." SET message_status = '0', message_term_expire = NULL, message_term_expire_action = NULL WHERE message_term_expire IS NOT NULL and message_term_expire <= ".$now." and message_term_expire_action = 'off'");
        $num_result1 = $this->db->Affected_Rows();
        $this->db->Execute("DELETE FROM ".PREFIX."_messages_".$this->registry->sitelang." WHERE message_term_expire IS NOT NULL and message_term_expire <= ".$now." and message_term_expire_action = 'delete'");
        $num_result2 = $this->db->Affected_Rows();
        if($num_result1 > 0 or $num_result2 > 0) {
    ...
    public function messages() {
        $sql = "SELECT message_id,message_title,message_content,message_notes,message_date,message_term_expire,message_value_view FROM ".PREFIX."_messages_".$this->registry->sitelang." WHERE message_status='1' order by message_date DESC";
        ...
        if(isset($row_exist) and $row_exist > 0) {
            while(!$result_messages->EOF) {
                ...
                } elseif($message_valueview == 1 and ($this->is_user() or $this->is_admin())) {
                    $message_alldata[] = array(
                        "message_id" => $message_id,
                        "message_title" => $message_title,
                        "message_content" => $message_content,
                ...
                } elseif($message_valueview == 2 and $this->is_admin()) {
                    $message_alldata[] = array(
                        "message_id" => $message_id,
                        "message_title" => $message_title,
                        "message_content" => $message_content,
                ...
                } elseif($message_valueview == 3 and (!$this->is_user() or $this->is_admin())) {
                    $message_alldata[] = array(
                        "message_id" => $message_id,
                        "message_title" => $message_title,
                        "message_content" => $message_content,
    ...
    public function block_rss($block_url,$block_refresh,$block_lastrefresh,$admin_rss_action) {
        ...
        if($admin_rss_action or (!$admin_rss_action and $block_lastrefresh < $past)) {
            $url = @parse_url($block_url);
            $fp = @fsockopen($url['host'],80,$errno,$errstr,20);
            if(!$fp) {
                return array('rss_link' => 'no');
            }
            ...
            if(isset($context)) {
                return array('rss_link' => $rsslink,'context' => $context);
            }
            return array('rss_link' => $rsslink);
        }
        return false;
    }
    ...
    public function update_block_rss(array $params) {
        $now = $this->get_time();
        if(!empty($params['case']) and $params['case'] === 'case1') {
            $block_content = $this->processing_data($params['cont']);
            $block_content = substr(substr($block_content,0,-1),1);
            $sql = "UPDATE ".PREFIX."_blocks_".$this->registry->sitelang." SET block_content='".$block_content."', block_last_refresh='".$now."' WHERE block_id='".$params['block_id']."'";
            $this->db->Execute($sql);
            $block_content = $this->extracting_data2($block_content);
            $rss_url = preg_replace("/http:\/\//i","",$params['block_url']);
            $rss_url = explode("/",$rss_url);
            return array('rss_url' => $rss_url[0],'block_content' => $block_content);
        } elseif(!empty($params['case']) and $params['case'] === 'case2') {
            $sql = "UPDATE ".PREFIX."_blocks_".$this->registry->sitelang." SET block_content='".$params['block_content']."', block_last_refresh='".$now."' WHERE block_id='".$params['block_id']."'";
            $this->db->Execute($sql);
            return true;
        } elseif(!empty($params['case']) and $params['case'] === 'case3') {
            $checkdb_needlobs = $this->check_db_need_lobs();
            if($checkdb_needlobs == 'yes') {
                $sql = "UPDATE ".PREFIX."_blocks_".$this->registry->sitelang." SET block_content=empty_clob(), block_last_refresh='".$now."' WHERE block_id='".$params['block_id']."'";
                $this->db->Execute($sql);
